...
{
    if(! slike(stype(path), {Notexist, Isfile}) || ! isdir(left(path, rinstr(path, "/") - 1))) return false;
    QFile file(path);
            ...
            while(! file.atEnd())
            {
                ...
                if(! cval.isEmpty() && ! cval.startsWith('#'))
                {
                    ...
                    else if(cline.startsWith("schedule="))
                    {
                        ...
                        for(uchar a(0) ; ! ilike(a, {vals.count(), 4}) ; ++a)
                        {
                            bool ok;
...
{
    struct stat fstat[2];
    if(ilike(-1, {stat(chr(file1), &fstat[0]), stat(chr(file2), &fstat[1])})) return 0;
...
{
    struct stat istat[2];
    if(ilike(-1, {stat(chr(srcitem), &istat[0]), stat(chr(newitem), &istat[1])})) return false;
...
{
    struct stat istat[2];
    if(ilike(-1, {lstat(chr(link1), &istat[0]), lstat(chr(link2), &istat[1])}) || ! S_ISLNK(istat[0].st_mode) || ! S_ISLNK(istat[1].st_mode) || istat[0].st_mtim.tv_sec != istat[1].st_mtim.tv_sec) return false;
...
void sb::run()
{
    ...
    case Readprttns:
    {
        for(cQStr &spath : QDir("/dev").entryList(QDir::System))
        {
            ...
            if(slike(path.length(), {8, 12}) && like(path, {"_/dev/sd*", "_/dev/hd*", "_/dev/mmcblk*"}) && devsize(path) > 536870911)
            {
    ...
    case Readlvdevs:
    {
        ...
        for(cQStr &item : QDir("/dev/disk/by-id").entryList(QDir::Files))
        {
            if(like(item, {"_usb-*", "_mmc-*"}) && islink("/dev/disk/by-id/" % item))
            {
                ...
                if(! path.isEmpty() && slike((path = "/dev/" % right(path, -6)).length(), {8, 12}) && like(path, {"_/dev/sd*", "_/dev/mmcblk*"}))
                {
                ...
                while(! in.atEnd())
                {
                    ...
                    if((mthd == 5 || (! like(item, {"_lost+found_", "_lost+found/*", "*/lost+found_", "*/lost+found/*", "_Systemback_", "_Systemback/*", "*/Systemback_", "*/Systemback/*", "*~_", "*~/*"}) && ! exclcheck(elist, item) && (macid.isEmpty() || ! item.contains(macid)))) && (! srcdir.isEmpty() || exist((mthd == 5 ? "/etc/skel/" : QStr("/home/" % usr % '/')) % item)))
                    {
                        ...
                        case Isdir:
                            ...
                            break;
                        case Isfile:
                            skppd = slike(mthd, {2, 3}) ? (QFile(srcdir % "/home/" % usr % '/' % item).size() > 8000000) : false;
        ...
        while(! in.atEnd())
        {
            ...
            if((mthd == 5 || (! like(item, {"_lost+found_", "_lost+found/*", "*/lost+found_", "*/lost+found/*", "_Systemback_", "_Systemback/*", "*/Systemback_", "*/Systemback/*", "*~_", "*~/*"}) && ! exclcheck(elist, item) && (macid.isEmpty() || ! item.contains(macid)))) && (! srcdir.isEmpty() || exist((mthd == 5 ? "/etc/skel/" : "/root/") % item)))
            {
                ...
                case Isdir:
                    ...
                    break;
                case Isfile:
                    skppd = slike(mthd, {2, 3}) ? (QFile(srcdir % "/root/" % item).size() > 8000000) : false;
