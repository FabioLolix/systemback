...
systemback::systemback(QWidget *parent)
    : QMainWindow(parent, Qt::FramelessWindowHint), ui(new Ui::systemback)
{
    ui->setupUi(this);
    ...
    if (!sb::like(font().family(), { "_Ubuntu_", "_FreeSans_" })
        || fontInfo().pixelSize() != 15 || fontInfo().weight() != QFont::Normal
        || fontInfo().bold() || fontInfo().italic() || fontInfo().overline()
        || fontInfo().strikeOut() || fontInfo().underline()) {
        sfctr = fontInfo().pixelSize() > 28 ? Max : fontInfo().pixelSize() > 21
                                                        ? High
                                                        : Normal;
        {
            while (sfctr > Normal
                   && (qApp->desktop()->width() - ss(30) < ss(698)
                       || qApp->desktop()->height() - ss(30) < ss(465)))
                sfctr = sfctr == Max ? High : Normal;
            QFont font;
            font.setFamily(QFontDatabase().families().contains("Ubuntu")
                               ? "Ubuntu"
                               : "FreeSans");
        ...
        if (sfctr > Normal) {
            for (QWidget *wdgt : findChildren<QWidget *>())
                wdgt->setGeometry(ss(wdgt->x()), ss(wdgt->y()),
                                  ss(wdgt->width()), ss(wdgt->height()));
            for (QPushButton *pbtn : findChildren<QPushButton *>())
                pbtn->setIconSize(QSize(ss(pbtn->iconSize().width()),
                                        ss(pbtn->iconSize().height())));
            ui->includeusers->setMinimumSize(ss(112), ss(32));
            ui->grubreinstallrestore->setMinimumSize(
                ui->includeusers->minimumSize());
            ui->grubreinstallrestoredisable->setMinimumSize(
                ui->includeusers->minimumSize());
            ui->grubinstallcopy->setMinimumSize(
                ui->includeusers->minimumSize());
            ui->grubinstallcopydisable->setMinimumSize(
                ui->includeusers->minimumSize());
            ui->grubreinstallrepair->setMinimumSize(
                ui->includeusers->minimumSize());
            ui->grubreinstallrepairdisable->setMinimumSize(
                ui->includeusers->minimumSize());
            ui->windowposition->setMinimumSize(ui->includeusers->minimumSize());
            ui->admins->setMinimumSize(ui->includeusers->minimumSize());
            ui->partitionsettings->verticalHeader()->setDefaultSectionSize(
                ss(20));
            ui->livedevices->verticalHeader()->setDefaultSectionSize(
                ui->partitionsettings->verticalHeader()->defaultSectionSize());
            QStyleOption optn;
            optn.init(ui->pointpipe1);
            QStr nsize(QStr::number(
                ss(ui->pointpipe1->style()
                       ->subElementRect(QStyle::SE_CheckBoxClickRect, &optn)
                       .width())));
            for (QCheckBox *ckbx : findChildren<QCheckBox *>())
                ckbx->setStyleSheet("QCheckBox::indicator{width:" % nsize
                                    % "px; height:" % nsize % "px;}");
            optn.init(ui->pnumber3);
            nsize = QStr::number(
                ss(ui->pnumber3->style()
                       ->subElementRect(QStyle::SE_RadioButtonClickRect, &optn)
                       .width()));
            for (QRadioButton *rbtn : findChildren<QRadioButton *>())
                rbtn->setStyleSheet("QRadioButton::indicator{width:" % nsize
                                    % "px; height:" % nsize % "px;}");
        }
    } else
        sfctr = Normal;

    if (qApp->arguments().count() == 3 && qApp->arguments().value(1)
                                          == "schedule") {
        sstart = true;
        dialog = 0;
    } else {
    ...
    connect(ui->windowmaximize, SIGNAL(Mouse_Pressed()), this,
            SLOT(wmaxpressed()));
    connect(ui->windowmaximize, SIGNAL(Mouse_Released()), this,
            SLOT(wmaxreleased()));
    ...
    connect(ui->windowminimize, SIGNAL(Mouse_Pressed()), this,
            SLOT(wminpressed()));
    connect(ui->windowminimize, SIGNAL(Mouse_Released()), this,
            SLOT(wminreleased()));
    ...
    connect(ui->windowclose, SIGNAL(Mouse_Released()), this,
            SLOT(wcreleased()));
    ...
    if (dialog > 0) {
        ...
        if (sb::isdir("/.systemback")
            && (sb::isfile("/cdrom/casper/filesystem.squashfs")
                || sb::isfile("/live/image/live/filesystem.squashfs")
                || sb::isfile(
        ...
        connect(ui->function1, SIGNAL(Mouse_Released()), this,
                SLOT(wreleased()));
        connect(ui->function1, SIGNAL(Mouse_DblClick()), this,
                SLOT(wdblclck()));
        connect(ui->function2, SIGNAL(Mouse_Pressed()), this, SLOT(wpressed()));
        connect(ui->function2, SIGNAL(Mouse_Move()), this, SLOT(wmove()));
        connect(ui->function2, SIGNAL(Mouse_Released()), this,
                SLOT(wreleased()));
        connect(ui->function4, SIGNAL(Mouse_Pressed()), this, SLOT(wpressed()));
        connect(ui->function4, SIGNAL(Mouse_Move()), this, SLOT(wmove()));
        connect(ui->function4, SIGNAL(Mouse_Released()), this,
                SLOT(wreleased()));
        connect(ui->windowbutton1, SIGNAL(Mouse_Enter()), this, SLOT(benter()));
        connect(ui->windowbutton1, SIGNAL(Mouse_Pressed()), this,
                SLOT(benter()));
        connect(ui->windowbutton2, SIGNAL(Mouse_Enter()), this, SLOT(benter()));
        connect(ui->windowbutton2, SIGNAL(Mouse_Pressed()), this,
                SLOT(benter()));
        connect(ui->windowbutton4, SIGNAL(Mouse_Enter()), this, SLOT(benter()));
        connect(ui->windowbutton4, SIGNAL(Mouse_Pressed()), this,
                SLOT(benter()));
        connect(ui->chooseresize, SIGNAL(Mouse_Enter()), this,
                SLOT(chssenter()));
        connect(ui->chooseresize, SIGNAL(Mouse_Leave()), this,
                SLOT(chssleave()));
        connect(ui->chooseresize, SIGNAL(Mouse_Pressed()), this,
                SLOT(chsspressed()));
        connect(ui->chooseresize, SIGNAL(Mouse_Released()), this,
                SLOT(chssreleased()));
        ...
        connect(ui->copyresize, SIGNAL(Mouse_Pressed()), this,
                SLOT(chsspressed()));
        connect(ui->copyresize, SIGNAL(Mouse_Released()), this,
                SLOT(chssreleased()));
        connect(ui->copyresize, SIGNAL(Mouse_Move()), this, SLOT(cpymove()));
        connect(ui->excluderesize, SIGNAL(Mouse_Enter()), this,
                SLOT(xcldenter()));
        connect(ui->excluderesize, SIGNAL(Mouse_Leave()), this,
                SLOT(xcldleave()));
        connect(ui->excluderesize, SIGNAL(Mouse_Pressed()), this,
                SLOT(chsspressed()));
        connect(ui->excluderesize, SIGNAL(Mouse_Released()), this,
                SLOT(chssreleased()));
        connect(ui->excluderesize, SIGNAL(Mouse_Move()), this,
                SLOT(xcldmove()));
        connect(ui->homepage1, SIGNAL(Mouse_Pressed()), this,
                SLOT(hmpg1pressed()));
        connect(ui->homepage1, SIGNAL(Mouse_Released()), this,
                SLOT(hmpg1released()));
        connect(ui->homepage1, SIGNAL(Mouse_Move()), this, SLOT(hmpg1move()));
        connect(ui->homepage2, SIGNAL(Mouse_Pressed()), this,
                SLOT(hmpg2pressed()));
        connect(ui->homepage2, SIGNAL(Mouse_Released()), this,
                SLOT(hmpg2released()));
        connect(ui->homepage2, SIGNAL(Mouse_Move()), this, SLOT(hmpg2move()));
        connect(ui->email, SIGNAL(Mouse_Pressed()), this, SLOT(emailpressed()));
        connect(ui->email, SIGNAL(Mouse_Released()), this,
                SLOT(emailreleased()));
        connect(ui->email, SIGNAL(Mouse_Move()), this, SLOT(emailmove()));
        connect(ui->donate, SIGNAL(Mouse_Pressed()), this, SLOT(dntpressed()));
        connect(ui->donate, SIGNAL(Mouse_Released()), this,
                SLOT(dntreleased()));
        ...
        connect(ui->partitionsettings, SIGNAL(Focus_In()), this,
                SLOT(finpsttngs()));
        connect(ui->partitionsettings, SIGNAL(Focus_Out()), this,
                SLOT(foutpsttngs()));
        connect(ui->usersettingscopy, SIGNAL(Mouse_Enter()), this,
                SLOT(center()));
        connect(ui->usersettingscopy, SIGNAL(Mouse_Leave()), this,
                SLOT(cleave()));
        connect(ui->umountdelete, SIGNAL(Mouse_Leave()), this,
                SLOT(umntleave()));
        ...
        if (qApp->arguments().count() == 3 && qApp->arguments().value(1)
            ...
            ui->adminstext->resize(fontMetrics().width(ui->adminstext->text())
                                   + ss(7),
                                   ui->adminstext->height());
            ui->admins->move(ui->adminstext->x() + ui->adminstext->width(),
                             ui->admins->y());
            ui->admins->setMaximumWidth(ui->passwordpanel->width()
                                        - ui->admins->x() - ss(8));
            ui->adminpasswordtext->resize(
                fontMetrics().width(ui->adminpasswordtext->text()) + ss(7),
                ui->adminpasswordtext->height());
            ui->adminpassword->move(ui->adminpasswordtext->x()
                                    + ui->adminpasswordtext->width(),
                                    ui->adminpassword->y());
            ui->adminpassword->resize(ss(336) - ui->adminpassword->x(),
                                      ui->adminpassword->height());
                    ...
                    while (!file.atEnd()) {
                        ...
                        if (sb::like(usrs, { "_sudo:*", "_admins:*" })) {
                            ...
                            for (cQStr &usr : usrs.split(','))
                                if (!usr.isEmpty() && ui->admins->findText(usr)
                                                      == -1)
            ...
            if (ui->admins->count() == 0)
                ...
                ui->admins->setCurrentIndex(
                    ui->admins->findText(qApp->arguments().value(2)));
            ...
            if (sstart) {
                ...
                connect(shdltimer, SIGNAL(timeout()), this,
                        SLOT(schedulertimer()));
...
systemback::~systemback()
{
    ...
    if (!nrxth) {
        ...
        if (xauth.startsWith("/tmp/sbXauthority-"))
            QFile::remove(xauth);
    }
...
{
    if (ui->statuspanel->isVisible() && !(sstart && sb::ThrdKill)
    ...
    if (!utblock) {
        ...
        if (!utimer->isActive()) {
            ...
            if (!sstart) {
                ...
                ui->partitiondelete->setStyleSheet(
                ...
                ui->partitionsettings->setHorizontalHeaderLabels(
                    { tr("Partition"), tr("Size"), tr("Label"),
                      tr("Current mount point"), tr("New mount point"),
                      tr("Filesystem"), tr("Format") });
                ...
                ui->partitionsettings->horizontalHeader()->setSectionResizeMode(
                    0, QHeaderView::Fixed);
                ui->partitionsettings->horizontalHeader()->setSectionResizeMode(
                    1, QHeaderView::Fixed);
                ui->partitionsettings->horizontalHeader()->setSectionResizeMode(
                ...
                ui->partitionsettings->horizontalHeader()->setSectionResizeMode(
                    6, QHeaderView::Fixed);
                ui->livedevices->setHorizontalHeaderLabels(
                    { tr("Partition"), tr("Size"), tr("Device"),
                      tr("Format") });
                ui->livedevices->horizontalHeader()->setSectionResizeMode(
                    0, QHeaderView::Fixed);
                ui->livedevices->horizontalHeader()->setSectionResizeMode(
                    1, QHeaderView::Fixed);
                ui->livedevices->horizontalHeader()->setSectionResizeMode(
                ...
                ui->repaircover->hide();
                if (sb::schdle[0] == "on")
                    ui->schedulerstate->click();
                if (sb::schdle[5] == "on")
                    ui->silentmode->setChecked(true);
                ui->windowposition->addItems({ tr("Top left"), tr("Top right"),
                                               tr("Center"), tr("Bottom left"),
                                               tr("Bottom right") });

                if (sb::schdle[6] == "topright")
                    ui->windowposition->setCurrentIndex(
                        ui->windowposition->findText(tr("Top right")));
                else if (sb::schdle[6] == "center")
                    ui->windowposition->setCurrentIndex(
                        ui->windowposition->findText(tr("Center")));
                else if (sb::schdle[6] == "bottomleft")
                    ui->windowposition->setCurrentIndex(
                        ui->windowposition->findText(tr("Bottom left")));
                else if (sb::schdle[6] == "bottomright")
                    ui->windowposition->setCurrentIndex(
                        ui->windowposition->findText(tr("Bottom right")));

                ui->schedulerday->setText(sb::schdle[1] % ' ' % tr("day(s)"));
                ui->schedulerhour->setText(sb::schdle[2] % ' ' % tr("hour(s)"));
                ui->schedulerminute->setText(sb::schdle[3] % ' '
                                             % tr("minute(s)"));
                ui->schedulersecond->setText(sb::schdle[4] % ' '
                                             % tr("seconds"));
                ui->includeuserstext->resize(
                    fontMetrics().width(ui->includeuserstext->text()) + ss(7),
                    ui->includeuserstext->height());
                ui->includeusers->move(ui->includeuserstext->x()
                                       + ui->includeuserstext->width(),
                                       ui->includeusers->y());
                ui->includeusers->setMaximumWidth(
                    width() - ui->includeusers->x() - ss(8));
                ui->grubreinstallrestoretext->resize(
                    fontMetrics().width(ui->grubreinstallrestoretext->text())
                    + ss(7),
                    ui->grubreinstallrestoretext->height());
                ui->grubreinstallrestore->move(
                    ui->grubreinstallrestoretext->x()
                    + ui->grubreinstallrestoretext->width(),
                    ui->grubreinstallrestore->y());
                ui->grubreinstallrestoredisable->move(
                    ui->grubreinstallrestore->x(),
                    ui->grubreinstallrestore->y());
                ui->grubinstalltext->resize(
                    fontMetrics().width(ui->grubinstalltext->text()) + ss(7),
                    ui->grubinstalltext->height());
                ui->grubinstallcopy->move(ui->grubinstalltext->x()
                                          + ui->grubinstalltext->width(),
                                          ui->grubinstallcopy->y());
                ui->grubinstallcopydisable->move(ui->grubinstallcopy->x(),
                                                 ui->grubinstallcopy->y());
                ui->grubreinstallrepairtext->resize(
                    fontMetrics().width(ui->grubreinstallrepairtext->text())
                    + ss(7),
                    ui->grubreinstallrepairtext->height());
                ui->grubreinstallrepair->move(
                    ui->grubreinstallrepairtext->x()
                    + ui->grubreinstallrepairtext->width(),
                    ui->grubreinstallrepair->y());
                ui->grubreinstallrepairdisable->move(
                    ui->grubreinstallrepair->x(), ui->grubreinstallrepair->y());
                ui->schedulerstatetext->resize(
                    fontMetrics().width(ui->schedulerstatetext->text()) + ss(7),
                    ui->schedulerstatetext->height());
                ui->schedulerstate->move(ui->schedulerstatetext->x()
                                         + ui->schedulerstatetext->width(),
                                         ui->schedulerstate->y());
                ui->schedulersecondtext->resize(
                    fontMetrics().width(ui->schedulersecondtext->text())
                    + ss(7),
                    ui->schedulersecondtext->height());
                ui->schedulersecondpanel->move(
                    ui->schedulersecondtext->x()
                    + ui->schedulersecondtext->width(),
                    ui->schedulersecondpanel->y());
                ui->windowpositiontext->resize(
                    fontMetrics().width(ui->windowpositiontext->text()) + ss(7),
                    ui->windowpositiontext->height());
                ui->windowposition->move(ui->windowpositiontext->x()
                                         + ui->windowpositiontext->width(),
                                         ui->windowposition->y());
                ui->homepage1->resize(fontMetrics().width(ui->homepage1->text())
                                      + ss(7),
                                      ui->homepage1->height());
                ui->homepage2->resize(fontMetrics().width(ui->homepage2->text())
                                      + ss(7),
                                      ui->homepage2->height());
                ui->email->resize(fontMetrics().width(ui->email->text())
                                  + ss(7),
                                  ui->email->height());
                ui->donate->resize(fontMetrics().width(ui->donate->text())
                                   + ss(7),
                                   ui->donate->height());
                ui->warning->resize(ui->warning->fontMetrics().width(
                                        ui->warning->text()) + ss(7),
                                    ui->warning->height());
                ui->format->resize(fontMetrics().width(ui->format->text())
                                   + ss(28),
                                   ui->format->height());
                ui->format->move((ui->partitionsettingspanel1->width()
                                  - ui->format->width()) / 2,
                                 ui->format->y());
                ui->fullrestore->resize(
                    fontMetrics().width(ui->fullrestore->text()) + ss(28),
                    ui->fullrestore->height());
                ui->systemrestore->resize(
                    fontMetrics().width(ui->systemrestore->text()) + ss(28),
                    ui->systemrestore->height());
                ui->configurationfilesrestore->resize(
                    fontMetrics().width(ui->configurationfilesrestore->text())
                    + ss(28),
                    ui->configurationfilesrestore->height());
                ui->keepfiles->resize(fontMetrics().width(ui->keepfiles->text())
                                      + ss(28),
                                      ui->keepfiles->height());
                ui->autorestoreoptions->resize(
                    fontMetrics().width(ui->autorestoreoptions->text())
                    + ss(28),
                    ui->autorestoreoptions->height());
                ui->skipfstabrestore->resize(
                    fontMetrics().width(ui->skipfstabrestore->text()) + ss(28),
                    ui->skipfstabrestore->height());
                ui->autorepairoptions->resize(
                    fontMetrics().width(ui->autorepairoptions->text()) + ss(28),
                    ui->autorepairoptions->height());
                ui->skipfstabrepair->resize(
                    fontMetrics().width(ui->skipfstabrepair->text()) + ss(28),
                    ui->skipfstabrepair->height());
                ui->userdatafilescopy->resize(
                    fontMetrics().width(ui->userdatafilescopy->text()) + ss(28),
                    ui->userdatafilescopy->height());
                ui->usersettingscopy->resize(
                    fontMetrics().width(tr(
                        "Transfer user configuration and data files")) + ss(28),
                    ui->usersettingscopy->height());
                ui->usersettingscopy->setCheckState(Qt::PartiallyChecked);
                ui->userdatainclude->resize(
                    fontMetrics().width(ui->userdatainclude->text()) + ss(28),
                    ui->userdatainclude->height());
                ui->systemrepair->resize(
                    fontMetrics().width(ui->systemrepair->text()) + ss(28),
                    ui->systemrepair->height());
                ui->fullrepair->resize(
                    fontMetrics().width(ui->fullrepair->text()) + ss(28),
                    ui->fullrepair->height());
                ui->grubrepair->resize(
                    fontMetrics().width(ui->grubrepair->text()) + ss(28),
                    ui->grubrepair->height());
                ui->pointexclude->resize(
                    fontMetrics().width(ui->pointexclude->text()) + ss(28),
                    ui->pointexclude->height());
                ui->liveexclude->resize(
                    fontMetrics().width(ui->liveexclude->text()) + ss(28),
                    ui->liveexclude->height());
                ui->silentmode->resize(
                    fontMetrics().width(ui->silentmode->text()) + ss(28),
                    ui->silentmode->height());
                ui->filesystem->addItems({ "ext4", "ext3", "ext2" });
                if (sb::execsrch("mkfs.btrfs"))
                    ui->filesystem->addItem("btrfs");
                if (sb::execsrch("mkfs.reiserfs"))
                    ui->filesystem->addItem("reiserfs");
                if (sb::execsrch("mkfs.jfs"))
                    ui->filesystem->addItem("jfs");
                if (sb::execsrch("mkfs.xfs"))
                    ui->filesystem->addItem("xfs");
                {
                    QFile file(":version");
                    file.open(QIODevice::ReadOnly);
                    ui->systembackversion->setText(
                        file.readLine().trimmed() % "_Qt"
                        % (QStr(qVersion()) == QStr(QT_VERSION_STR)
                               ? QStr(qVersion())
                               : QStr(qVersion()) % '(' % QStr(QT_VERSION_STR)
                                 % ')') % '_' % sb::getarch());
                }
                ui->repairmountpoint->addItems(
                    { nullptr, "/mnt", "/mnt/home", "/mnt/boot" });
                ...
                ui->repairmountpoint->addItems(
                    { "/mnt/usr", "/mnt/var", "/mnt/opt", "/mnt/usr/local" });
            ...
            if (sb::like(
                    prun,
                    { '_' % tr("Creating restore point") % '_',
                      '_' % tr("Restoring the full system") % '_',
                      '_' % tr("Restoring the system files") % '_',
                      '_' % tr("Restoring user(s) configuration files") % '_',
                      '_' % tr("Repairing the system files") % '_',
                      '_' % tr("Repairing the full system") % '_',
                      '_' % tr("Copying the system") % '_',
                      '_' % tr("Installing the system") % '_',
                      '_' % tr("Creating Live system") % '\n' % tr("process")
                      % " 3/3_",
                      '_' % tr("Writing Live image to USB device") % '_',
                      '_' % tr("Converting Live system image") % "\n*" })) {
                if (!ui->interrupt->isEnabled())
                ...
                if (cperc != -1) {
                    if (ui->progressbar->maximum() == 0)
                        ui->progressbar->setMaximum(100);
                    if (cperc < 101 && ui->progressbar->value() < cperc)
                        ui->progressbar->setValue(cperc);
                } else if (ui->progressbar->maximum() == 100)
                    ui->progressbar->setMaximum(0);
            } else {
                if (sb::like(
                        prun,
                        { '_' % tr("Deleting restore point") % "*",
                          '_' % tr("Deleting old restore point") % "*",
                          '_' % tr("Deleting incomplete restore point") % '_',
                          '_' % tr("Creating Live system") % "\n*" })) {
                    if (!ui->interrupt->isEnabled())
                        ui->interrupt->setEnabled(true);
                } else if (ui->interrupt->isEnabled())
            ...
            if (sb::isdir(sb::sdir[1]) && sb::access(sb::sdir[1], sb::Write)) {
                ...
                if (ppipe == 0 && pname == tr("Currently running system")
                    && !ui->newrestorepoint->isEnabled())
                    ui->newrestorepoint->setEnabled(true);

                if ((ui->point1->isEnabled()
                     && !sb::isdir(sb::sdir[1] % "/S01_" % sb::pnames[0]))
                    || (ui->point2->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S02_" % sb::pnames[1]))
                    || (ui->point3->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S03_" % sb::pnames[2]))
                    || (ui->point4->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S04_" % sb::pnames[3]))
                    || (ui->point5->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S05_" % sb::pnames[4]))
                    || (ui->point6->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S06_" % sb::pnames[5]))
                    || (ui->point7->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S07_" % sb::pnames[6]))
                    || (ui->point8->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S08_" % sb::pnames[7]))
                    || (ui->point9->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S09_" % sb::pnames[8]))
                    || (ui->point10->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/S10_" % sb::pnames[9]))
                    || (ui->point11->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/H01_" % sb::pnames[10]))
                    || (ui->point12->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/H02_" % sb::pnames[11]))
                    || (ui->point13->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/H03_" % sb::pnames[12]))
                    || (ui->point14->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/H04_" % sb::pnames[13]))
                    || (ui->point15->isEnabled()
                        && !sb::isdir(sb::sdir[1] % "/H05_" % sb::pnames[14])))
                    accesserror();
            } else {
                if (ui->point1->isEnabled() || ui->pointpipe11->isEnabled())
                    accesserror();
                if (ui->newrestorepoint->isEnabled())
                    ui->newrestorepoint->setDisabled(true);

                if (ui->storagedirarea->styleSheet().isEmpty()) {
                    ui->storagedirarea->setStyleSheet(
                        "background-color: rgb(255, 103, 103)");
                    ui->storagedir->setStyleSheet(
                        "background-color: rgb(255, 103, 103)");
                    fontcheck(Strgdr);
                }
            }
            ...
            if (ui->installpanel->isVisible()) {
                if (ui->installmenu->isEnabled()
                    && ui->fullnamepipe->isVisible()
                    && ui->usernamepipe->isVisible()
                    && ui->hostnamepipe->isVisible()
                    && ui->passwordpipe->isVisible()
                    && (ui->rootpassword1->text().isEmpty()
                        || ui->rootpasswordpipe->isVisible())
                    && !ui->installnext->isEnabled())
                    ui->installnext->setEnabled(true);
            } else if (ui->livecreatepanel->isVisible()) {
                if (ui->livenameerror->isVisible()
                    || ui->liveworkdir->text().isEmpty()) {
                    if (ui->livecreatenew->isEnabled())
                        ui->livecreatenew->setDisabled(true);
                } else if (ui->livenamepipe->isVisible() || ui->livename->text()
                                                            == "auto") {
                    if (sb::isdir(sb::sdir[2])
                        && sb::access(sb::sdir[2], sb::Write)) {
                        ...
                        if (ickernel && !ui->livecreatenew->isEnabled())
                            ui->livecreatenew->setEnabled(true);
                        ...
                        if (ui->liveworkdirarea->styleSheet().isEmpty()) {
                            ui->liveworkdirarea->setStyleSheet(
                                "background-color: rgb(255, 103, 103)");
                            ui->liveworkdir->setStyleSheet(
                                "background-color: rgb(255, 103, 103)");
                            fontcheck(Lvwrkdr);
                        }
                        ...
                        if (ui->livecreatenew->isEnabled())
                            ui->livecreatenew->setDisabled(true);
                        if (ui->livedelete->isEnabled())
                            ui->livedelete->setDisabled(true);
                        if (ui->liveconvert->isEnabled())
                            ui->liveconvert->setDisabled(true);
                        if (ui->livewritestart->isEnabled())
                            ui->livewritestart->setDisabled(true);

                        if (ui->livelist->isEnabled()) {
                            if (ui->livelist->count() > 0)
                                ui->livelist->clear();
                ...
                if (sb::mcheck("/mnt")) {
                    if (ui->grubrepair->isChecked()) {
                        if (sb::execsrch("update-grub2")
                            && sb::isfile("/mnt/var/lib/dpkg/info/grub-" % grub
                                          % ".list")) {
                            ...
                            if (ui->repairnext->isEnabled())
                                ui->repairnext->setDisabled(true);
                        }
                    } else if (!ui->repairnext->isEnabled())
                        ui->repairnext->setEnabled(true);
                } else {
                    if (ui->grubrepair->isChecked()
                        && ui->grubreinstallrepair->isVisible()) {
                        ui->grubreinstallrepair->hide();
                        ui->grubreinstallrepairdisable->show();
                    }
...
void systemback::fontcheck(uchar wdgt)
{
    ...
    case Lvwrkdr:
        ...
        break;
    case Dpath:
        if (ui->dirpath->font().pixelSize() != ss(15))
            ui->dirpath->setFont(font());
        break;
    case Rpnts:
        for (QWidget *lndt : ui->sbpanel->findChildren<QLineEdit *>())
            if (lndt->font().pixelSize() != ss(15))
                lndt->setFont(font());
    }
}
...
{
    return QCursor::pos().x() < pos().x() + wpos.x()
               || QCursor::pos().y() < pos().y() + wpos.y()
               || QCursor::pos().x() > pos().x() + wpos.x() + wgeom.width()
               || QCursor::pos().y() > pos().y() + wpos.y() + wgeom.height()
               ? false
               : true;
}
...
{
    if (!uchkd
        && (ui->admins->count() == 0 || ui->admins->currentText() == "root")) {
        QSL usrs;

        {
            ...
            if (file.open(QIODevice::ReadOnly))
                while (!file.atEnd()) {
                    ...
                    if (sb::like(usr, { "*:x:1000:10*", "*:x:1001:10*",
                                        "*:x:1002:10*", "*:x:1003:10*",
                                        "*:x:1004:10*", "*:x:1005:10*",
                                        "*:x:1006:10*", "*:x:1007:10*",
                                        "*:x:1008:10*", "*:x:1009:10*",
                                        ...
                                        "*:x:1012:10*", "*:x:1013:10*",
                                        "*:x:1014:10*", "*:x:1015:10*" }))
                        usrs.append(sb::left(usr, sb::instr(usr, ":") - 1));
                }
        }
...
void systemback::buttonstimer()
{
    if (minside(ui->buttonspanel->pos(), ui->buttonspanel->geometry())) {
        if (ui->windowmaximize->isVisible()) {
            if (minside(ui->buttonspanel->pos() + ui->windowmaximize->pos(),
                        ui->windowmaximize->geometry())) {
                if (ui->windowmaximize->backgroundRole()
                    == QPalette::Foreground) {
                    ui->windowmaximize->setBackgroundRole(QPalette::Background);
                    ui->windowmaximize->setForegroundRole(QPalette::Text);
                }
            } else if (ui->windowmaximize->backgroundRole()
                       == QPalette::Background) {
                ui->windowmaximize->setBackgroundRole(QPalette::Foreground);
                ui->windowmaximize->setForegroundRole(QPalette::Base);
            }
        }
        if (minside(ui->buttonspanel->pos() + ui->windowminimize->pos(),
                    ui->windowminimize->geometry())) {
        ...
        } else if (ui->windowminimize->backgroundRole()
                   == QPalette::Background) {
            ui->windowminimize->setBackgroundRole(QPalette::Foreground);
            ui->windowminimize->setForegroundRole(QPalette::Base);
        }
        if (ui->windowclose->isVisible()) {
            if (minside(ui->buttonspanel->pos() + ui->windowclose->pos(),
                        ui->windowclose->geometry())) {
            ...
            } else if (ui->windowclose->backgroundRole()
                       == QPalette::Background) {
                ui->windowclose->setBackgroundRole(QPalette::Foreground);
                ui->windowclose->setForegroundRole(QPalette::Base);
            }
...
void systemback::schedulertimer()
{
    if (ui->schedulernumber->text().isEmpty())
        ...
        ui->schedulernumber->setText(
            QStr::number(sb::left(ui->schedulernumber->text(), -1).toShort()
                         - 1) % 's');
}

void systemback::dialogtimer()
{
    ui->dialognumber->setText(QStr::number(sb::left(ui->dialognumber->text(),
                                                    -1).toShort() - 1) % "s");
    if (ui->dialognumber->text() == "0s"
        && sb::like(ui->dialogok->text(),
                    { '_' % tr("Reboot") % '_', '_' % tr("X restart") % '_' }))
        on_dialogok_clicked();
}
...
{
    if (ui->copypanel->isVisible() || ui->excludepanel->isVisible()
        || ui->choosepanel->isVisible()) {
        if (size() == qApp->desktop()->availableGeometry().size()) {
            setGeometry(wgeom[4], wgeom[5], wgeom[2], wgeom[3]);
            setMaximumSize(
                qApp->desktop()->availableGeometry().width() - ss(60),
                qApp->desktop()->availableGeometry().height() - ss(60));
        } else {
            wgeom[4] = wgeom[0];
            wgeom[5] = wgeom[1];
...
void systemback::benter()
{
    if (ui->statuspanel->isVisible()) {
        ...
        if (ui->windowclose->isVisible())
            ui->windowclose->hide();
        if (ui->buttonspanel->width() != ss(48))
            ui->buttonspanel->resize(ss(48), ss(48));
    } else if (ui->copypanel->isVisible() || ui->excludepanel->isVisible()
               || ui->choosepanel->isVisible()) {
        ...
        if (ui->windowclose->x() != ss(92))
            ui->windowclose->move(ss(92), ss(2));
        if (ui->windowclose->isHidden())
            ui->windowclose->show();
        if (ui->buttonspanel->width() != ss(138))
            ui->buttonspanel->resize(ss(138), ss(48));
    } else {
        if (ui->windowmaximize->isVisibleTo(ui->buttonspanel)) {
            ui->windowmaximize->hide();
            ui->windowminimize->move(ss(2), ss(2));
        ...
        if (ui->windowclose->x() != ss(47))
            ui->windowclose->move(ss(47), ss(2));
        if (ui->windowclose->isHidden())
            ui->windowclose->show();
        if (ui->buttonspanel->width() != ss(93))
            ui->buttonspanel->resize(ss(93), ss(48));
    }

    if (ui->buttonspanel->x() != width() - ui->buttonspanel->width())
        ui->buttonspanel->move(width() - ui->buttonspanel->width(), 0);
...
{
    if (ui->buttonspanel->isVisible() && ui->windowmaximize->foregroundRole()
                                         == QPalette::Highlight) {
        if (ui->windowmaximize->text() == "□") {
            wgeom[4] = wgeom[0];
            wgeom[5] = wgeom[1];
...
{
    if (ui->buttonspanel->isVisible() && ui->windowminimize->foregroundRole()
                                         == QPalette::Highlight)
        showMinimized();
}
...
{
    if (ui->buttonspanel->isVisible() && ui->windowclose->foregroundRole()
                                         == QPalette::Highlight)
        close();
}

void systemback::apokkeyreleased() { on_passwordinputok_clicked(); }

void systemback::chssenter()
{
    if (size() != qApp->desktop()->availableGeometry().size()
        && ui->chooseresize->width() == ss(10))
        ui->chooseresize->setGeometry(ui->chooseresize->x() - ss(20),
                                      ui->chooseresize->y() - ss(20), ss(30),
                                      ss(30));
}

void systemback::chssleave()
{
    if (ui->chooseresize->width() == ss(30) && !qApp->overrideCursor())
        ui->chooseresize->setGeometry(ui->chooseresize->x() + ss(20),
                                      ui->chooseresize->y() + ss(20), ss(10),
                                      ss(10));
}
...
{
    if (size() != qApp->desktop()->availableGeometry().size()) {
        wgeom[2] = QCursor::pos().x() - x() + width() - ui->chooseresize->x()
                   - ui->chooseresize->MouseX - ss(1);
        wgeom[3] = QCursor::pos().y() - y() + height() - ui->chooseresize->y()
                   - ui->chooseresize->MouseY - ss(24);
        if (width() != wgeom[2] || height() != wgeom[3])
            resize(wgeom[2], wgeom[3]);
    }
}
...
{
    if (size() != qApp->desktop()->availableGeometry().size()
        && ui->copyresize->width() == ss(10))
        ui->copyresize->setGeometry(ui->copyresize->x() - ss(20),
                                    ui->copyresize->y() - ss(20), ss(30),
                                    ss(30));
}

void systemback::cpyleave()
{
    if (ui->copyresize->width() == ss(30) && !qApp->overrideCursor())
        ui->copyresize->setGeometry(ui->copyresize->x() + ss(20),
                                    ui->copyresize->y() + ss(20), ss(10),
                                    ss(10));
}
...
{
    if (size() != qApp->desktop()->availableGeometry().size()) {
        wgeom[2] = QCursor::pos().x() - x() + width() - ui->copyresize->x()
                   - ui->copyresize->MouseX - ss(1);
        wgeom[3] = QCursor::pos().y() - y() + height() - ui->copyresize->y()
                   - ui->copyresize->MouseY - ss(24);
        if (width() != wgeom[2] || height() != wgeom[3])
            resize(wgeom[2], wgeom[3]);
    }
}
...
{
    if (size() != qApp->desktop()->availableGeometry().size()
        && ui->excluderesize->width() == ss(10))
        ui->excluderesize->setGeometry(ui->excluderesize->x() - ss(20),
                                       ui->excluderesize->y() - ss(20), ss(30),
                                       ss(30));
}

void systemback::xcldleave()
{
    if (ui->excluderesize->width() == ss(30) && !qApp->overrideCursor())
        ui->excluderesize->setGeometry(ui->excluderesize->x() + ss(20),
                                       ui->excluderesize->y() + ss(20), ss(10),
                                       ss(10));
}
...
{
    if (size() != qApp->desktop()->availableGeometry().size()) {
        wgeom[2] = QCursor::pos().x() - x() + width() - ui->excluderesize->x()
                   - ui->excluderesize->MouseX - ss(1);
        wgeom[3] = QCursor::pos().y() - y() + height() - ui->excluderesize->y()
                   - ui->excluderesize->MouseY - ss(24);
        if (width() != wgeom[2] || height() != wgeom[3])
            resize(wgeom[2], wgeom[3]);
    }
}
...
void systemback::hmpg1released()
{
    if (ui->homepage1->foregroundRole() == QPalette::Highlight) {
        ...
        sb::exec(
            "su -c \"xdg-open https://sourceforge.net/projects/systemback &\" "
            % guname(),
            nullptr, false, true);
    }
}
...
{
    if (minside(ui->aboutpanel->pos() + ui->homepage1->pos(),
                ui->homepage1->geometry())) {
        if (ui->homepage1->foregroundRole() == QPalette::Text)
            ui->homepage1->setForegroundRole(QPalette::Highlight);
...
void systemback::hmpg2released()
{
    if (ui->homepage2->foregroundRole() == QPalette::Highlight) {
        ...
        sb::exec("su -c \"xdg-open https://launchpad.net/systemback &\" "
                 % guname(),
                 nullptr, false, true);
    }
}
...
{
    if (minside(ui->aboutpanel->pos() + ui->homepage2->pos(),
                ui->homepage2->geometry())) {
        if (ui->homepage2->foregroundRole() == QPalette::Text)
            ui->homepage2->setForegroundRole(QPalette::Highlight);
...
void systemback::emailreleased()
{
    if (ui->email->foregroundRole() == QPalette::Highlight) {
        ...
        sb::exec("su -c \"xdg-email nemh@freemail.hu &\" " % guname(), nullptr,
                 false, true);
    }
}
...
{
    if (minside(ui->aboutpanel->pos() + ui->email->pos(),
                ui->email->geometry())) {
        if (ui->email->foregroundRole() == QPalette::Text)
            ui->email->setForegroundRole(QPalette::Highlight);
...
void systemback::dntreleased()
{
    if (ui->donate->foregroundRole() == QPalette::Highlight) {
        ...
        sb::exec("su -c \"xdg-open "
                 "'https://www.paypal.com/cgi-bin/"
                 "webscr?cmd=_s-xclick&hosted_button_id=ZQ668BBR7UCEQ' &\" "
                 % guname(),
                 nullptr, false, true);
    }
}
...
{
    if (minside(ui->aboutpanel->pos() + ui->donate->pos(),
                ui->donate->geometry())) {
        if (ui->donate->foregroundRole() == QPalette::Text)
            ui->donate->setForegroundRole(QPalette::Highlight);
...
void systemback::finpsttngs()
{
    if (ui->partitionsettingspanel2->isVisible())
        for (ushort a(ui->partitionsettings->currentRow() + 1);
             a < ui->partitionsettings->rowCount()
             && ui->partitionsettings->item(a, 0)->background()
                == QPalette().color(QPalette::Inactive, QPalette::Highlight);
             ++a) {
            ui->partitionsettings->item(a, 0)
                ->setBackground(QPalette().highlight());
            ui->partitionsettings->item(a, 0)
                ->setForeground(QPalette().highlightedText());
        }
}
    ...
    if (ui->partitionsettingspanel2->isVisibleTo(ui->copypanel))
        for (ushort a(ui->partitionsettings->currentRow() + 1);
             ...
             && ui->partitionsettings->item(a, 0)->background()
                == QPalette().highlight();
             ++a) {
            ui->partitionsettings->item(a, 0)->setBackground(
                QPalette().color(QPalette::Inactive, QPalette::Highlight));
            ui->partitionsettings->item(a, 0)->setForeground(QPalette().color(
...
{
    if (ui->usersettingscopy->checkState() == Qt::PartiallyChecked)
        ui->usersettingscopy->setText(
            tr("Transfer user configuration and data files"));
}

void systemback::cleave()
{
    if (ui->usersettingscopy->checkState() == Qt::PartiallyChecked
        && ui->usersettingscopy->text()
           == tr("Transfer user configuration and data files"))
...
{
    if (!ui->umountdelete->isEnabled() && ui->umountdelete->text()
                                          == tr("! Delete !"))
...
{
    ui->usersettingscopy->setText(
        arg1 == 1 ? tr("Transfer user configuration files")
                  : tr("Transfer user configuration and data files"));
}
...
void systemback::pointupgrade()
{
    ...
    if (!sb::pnames[0].isEmpty()) {
        ...
        if (ui->point1->text() != tr("empty"))
            ui->point1->setText(tr("empty"));
    }
    ...
    if (!sb::pnames[1].isEmpty()) {
        ...
        if (ui->point2->text() != tr("empty"))
            ui->point2->setText(tr("empty"));
    }

    if (!sb::pnames[2].isEmpty()) {
        if (!ui->point3->isEnabled()) {
            ui->point3->setEnabled(true);
            if (sb::pnumber == 3 && ui->point3->styleSheet().isEmpty())
                ui->point3->setStyleSheet(
                    "background-color: rgb(255, 103, 103)");
            ui->point3->setText(sb::pnames[2]);
        ...
        if (!ui->point3->styleSheet().isEmpty())
            ui->point3->setStyleSheet(nullptr);
        if (ui->point3->text() != tr("empty"))
            ui->point3->setText(tr("empty"));
    }

    if (!sb::pnames[3].isEmpty()) {
        if (!ui->point4->isEnabled()) {
            ui->point4->setEnabled(true);
            if (sb::pnumber < 5 && ui->point4->styleSheet().isEmpty())
                ui->point4->setStyleSheet(
                    "background-color: rgb(255, 103, 103)");
            ui->point4->setText(sb::pnames[3]);
        ...
        if (!ui->point4->styleSheet().isEmpty())
            ui->point4->setStyleSheet(nullptr);

        if (sb::pnumber == 3) {
            if (ui->point4->text() != tr("not used"))
    ...
    if (!sb::pnames[4].isEmpty()) {
        if (!ui->point5->isEnabled()) {
            ...
            if (sb::pnumber < 6 && ui->point5->styleSheet().isEmpty())
                ui->point5->setStyleSheet(
                    "background-color: rgb(255, 103, 103)");
            ui->point5->setText(sb::pnames[4]);
        ...
        if (!ui->point5->styleSheet().isEmpty())
            ui->point5->setStyleSheet(nullptr);

        if (sb::pnumber < 5) {
            if (ui->point5->text() != tr("not used"))
    ...
    if (!sb::pnames[5].isEmpty()) {
        if (!ui->point6->isEnabled()) {
            ...
            if (sb::pnumber < 7 && ui->point6->styleSheet().isEmpty())
                ui->point6->setStyleSheet(
                    "background-color: rgb(255, 103, 103)");
            ui->point6->setText(sb::pnames[5]);
        ...
        if (!ui->point6->styleSheet().isEmpty())
            ui->point6->setStyleSheet(nullptr);

        if (sb::pnumber < 6) {
            if (ui->point6->text() != tr("not used"))
    ...
    if (!sb::pnames[6].isEmpty()) {
        if (!ui->point7->isEnabled()) {
            ...
            if (sb::pnumber < 8 && ui->point7->styleSheet().isEmpty())
                ui->point7->setStyleSheet(
                    "background-color: rgb(255, 103, 103)");
            ui->point7->setText(sb::pnames[6]);
        ...
        if (!ui->point7->styleSheet().isEmpty())
            ui->point7->setStyleSheet(nullptr);

        if (sb::pnumber < 7) {
            if (ui->point7->text() != tr("not used"))
    ...
    if (!sb::pnames[7].isEmpty()) {
        if (!ui->point8->isEnabled()) {
            ...
            if (sb::pnumber < 9 && ui->point8->styleSheet().isEmpty())
                ui->point8->setStyleSheet(
                    "background-color: rgb(255, 103, 103)");
            ui->point8->setText(sb::pnames[7]);
        ...
        if (!ui->point8->styleSheet().isEmpty())
            ui->point8->setStyleSheet(nullptr);

        if (sb::pnumber < 8) {
            if (ui->point8->text() != tr("not used"))
    ...
    if (!sb::pnames[8].isEmpty()) {
        if (!ui->point9->isEnabled()) {
            ...
            if (sb::pnumber < 10 && ui->point9->styleSheet().isEmpty())
                ui->point9->setStyleSheet(
                    "background-color: rgb(255, 103, 103)");
            ui->point9->setText(sb::pnames[8]);
        ...
        if (!ui->point9->styleSheet().isEmpty())
            ui->point9->setStyleSheet(nullptr);

        if (sb::pnumber < 9) {
            if (ui->point9->text() != tr("not used"))
    ...
    if (!sb::pnames[9].isEmpty()) {
        ...
        if (!ui->point10->styleSheet().isEmpty())
            ui->point10->setStyleSheet(nullptr);

        if (sb::pnumber < 10) {
            if (ui->point10->text() != tr("not used"))
    ...
    if (!sb::pnames[10].isEmpty()) {
        ...
        if (ui->point11->text() != tr("empty"))
            ui->point11->setText(tr("empty"));
    }
    ...
    if (!sb::pnames[11].isEmpty()) {
        ...
        if (ui->point12->text() != tr("empty"))
            ui->point12->setText(tr("empty"));
    }
    ...
    if (!sb::pnames[12].isEmpty()) {
        ...
        if (ui->point13->text() != tr("empty"))
            ui->point13->setText(tr("empty"));
    }
    ...
    if (!sb::pnames[13].isEmpty()) {
        ...
        if (ui->point14->text() != tr("empty"))
            ui->point14->setText(tr("empty"));
    }
    ...
    if (!sb::pnames[14].isEmpty()) {
        ...
        if (ui->point15->text() != tr("empty"))
            ui->point15->setText(tr("empty"));
    }
...
void systemback::accesserror()
{
    ...
    if (ui->dialogquestion->isVisible())
        ...
        on_copyback_clicked();
        if (ui->function1->text() != "Systemback")
...
start:
    ...
    if (mthd < 3) {
        if (sb::isfile("/etc/fstab") && sb::isfile(sb::sdir[1] % '/' % cpoint
                                                   % '_' % pname % "/etc/fstab")
            && sb::fload("/etc/fstab")
               == sb::fload(sb::sdir[1] % '/' % cpoint % '_' % pname
                            % "/etc/fstab")) {
            fcmp = true;
    ...
    if (sb::srestore(mthd, ui->includeusers->currentText(),
                     sb::sdir[1] % '/' % cpoint % '_' % pname, nullptr,
                     sfstab)) {
        if (intrrpt)
            goto exit;
        sb::Progress = -1;
        ...
        if (mthd < 3) {
            if (ui->grubreinstallrestore->isVisibleTo(ui->restorepanel)
                && (!ui->grubreinstallrestore->isEnabled()
                    || ui->grubreinstallrestore->currentText()
                       != tr("Disabled"))) {
                uchar rv(0);

                if (ui->autorestoreoptions->isChecked()
                    || ui->grubreinstallrestore->currentText() == "Auto") {
                    if (fcmp)
                        sb::exec("update-grub");
                    else {
                        sb::exec("update-grub");
...
start:
    ...
    if (mthd == 0) {
        ...
        if (ui->grubreinstallrepair->currentText() == "Auto") {
            ...
            sb::crtfile("/mnt/grubinst",
                        "#!/bin/sh\nupdate-grub\ngrub-install --force "
                        % sb::left(mntdev, 8) % '\n');
        } else
            sb::crtfile("/mnt/grubinst",
                        "#!/bin/sh\nupdate-grub\ngrub-install --force "
                        % ui->grubreinstallrepair->currentText() % '\n');
        ...
        if (mthd < 3) {
            if (sb::isfile("/mnt/etc/fstab")
                && sb::isfile(sb::sdir[1] % '/' % cpoint % '_' % pname
                              % "/etc/fstab")
                && sb::fload("/mnt/etc/fstab")
                   == sb::fload(sb::sdir[1] % '/' % cpoint % '_' % pname
                                % "/etc/fstab")) {
                fcmp = true;
                if (ui->autorepairoptions->isChecked())
                    sfstab = true;
            }

            if (!ui->autorepairoptions->isChecked()
                && ui->skipfstabrepair->isChecked())
                sfstab = true;
        }
        ...
        if (pname == tr("Live image")) {
            if (!sb::isdir("/.systembacklivepoint")
                && !QDir().mkdir("/.systembacklivepoint")) {
                QFile::rename("/.systembacklivepoint",
                              "/.systembacklivepoint_" % sb::rndstr());
                QDir().mkdir("/.systembacklivepoint");
            }
            ...
            if (intrrpt)
                goto exit;
            rv = sb::srestore(mthd, nullptr, "/.systembacklivepoint", "/mnt",
                              sfstab);
            sb::umount("/.systembacklivepoint");
            QDir().rmdir("/.systembacklivepoint");
        } else
            rv = sb::srestore(mthd, nullptr,
                              sb::sdir[1] % '/' % cpoint % '_' % pname, "/mnt",
                              sfstab);
        ...
        if (rv) {
            if (ui->grubreinstallrepair->isVisibleTo(ui->repairpanel)
                ...
                if (ui->autorepairoptions->isChecked()
                    || ui->grubreinstallrepair->currentText() == "Auto") {
                    if (fcmp)
                        sb::crtfile("/mnt/grubinst",
                                    "#!/bin/sh\nupdate-grub\n");
                    else {
                ...
                if (intrrpt)
                    goto exit;
                if (sb::exec("chroot /mnt /grubinst") > 0)
                    dialog = ui->fullrepair->isChecked() ? 24 : 11;
                QFile::remove("/mnt/grubinst");
                sb::umount("/mnt/dev");
                sb::umount("/mnt/dev/pts");
                sb::umount("/mnt/proc");
            ...
            if (sb::ilike(dialog, { 5, 6, 41 })) {
                if (ppipe == 1 && sb::isdir(sb::sdir[1])
                    && sb::access(sb::sdir[1], sb::Write))
                    sb::crtfile(sb::sdir[1] % "/.sbschedule");
...
error:
    ...
    if (sb::ilike(dialog, { 22, 34 })) {
        ...
        if (sb::dfree("/.sbsystemcopy") > 104857600
            && (!sb::isdir("/.sbsystemcopy/home")
                || sb::dfree("/.sbsystemcopy/home") > 104857600)
            && (!sb::isdir("/.sbsystemcopy/boot")
                || sb::dfree("/.sbsystemcopy/boot") > 52428800)) {
            if (!sb::ThrdDbg.isEmpty())
                printdbgmsg();
            dialog = ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 39
                                                                       : 40;
        } else
            dialog = ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 21
                                                                       : 35;
    }
...

    {
        ...
        while (!in.atEnd())
            mnts[1].prepend(in.readLine() % '\n');
        in.setString(&mnts[1], QIODevice::ReadOnly);

        while (!in.atEnd()) {
            QStr cline(in.readLine());
            if (sb::like(cline,
                         { "* /.sbsystemcopy*", "* /.systembacklivepoint *" }))
                sb::umount(sb::left(cline, sb::instr(cline, " ") - 1));
        }
    }
...
start:
    statustart();
    prun = ui->userdatafilescopy->isVisibleTo(ui->copypanel)
               ? tr("Copying the system")
               : tr("Installing the system");
    QSL msort;

    for (ushort a(0); a < ui->partitionsettings->rowCount(); ++a)
        if (!ui->partitionsettings->item(a, 4)->text().isEmpty()
            && (ui->partitionsettings->item(a, 4)->text() != "/home"
                || ui->partitionsettings->item(a, 3)->text().isEmpty()))
            msort.append(ui->partitionsettings->item(a, 4)->text()
                         % (ui->partitionsettings->item(a, 6)->text() == "x"
                                ? QStr('\n' % ui->partitionsettings->item(a, 5)
                                                  ->text() % '\n')
                                : "\n-\n")
                         % ui->partitionsettings->item(a, 0)->text());

    msort.sort();
    if (!sb::isdir("/.sbsystemcopy") && !QDir().mkdir("/.sbsystemcopy"))
        goto error;
    ushort rv;
    ...
    for (cQStr &vals : msort) {
        ...
        if (fstype != "-") {
            QStr lbl("SB@" % (mpoint.startsWith('/') ? sb::right(mpoint, -1)
                                                     : mpoint));
            ...
            if (fstype == "swap")
                ...
                rv = sb::setpflag(part, "boot")
                         ? sb::exec("mkfs.vfat -F 32 -n " % lbl.toUpper() % ' '
                                    % part)
                         : 255;
            else if (fstype == "btrfs") {
                rv = sb::exec("mkfs.btrfs -fL " % lbl % ' ' % part);
                if (rv > 0)
                    rv = sb::exec("mkfs.btrfs -L " % lbl % ' ' % part);
            } else
            ...
            if (rv > 0) {
                dialogdev = part;
                dialog = ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 36
                                                                           : 52;
                goto error;
            }
        }
        ...
        if (mpoint != "SWAP") {
            ...
            if (sb::mount(part, "/.sbsystemcopy" % mpoint)) {
                if (fstype == "btrfs") {
                    sb::exec("btrfs subvolume create /.sbsystemcopy" % mpoint
                             % "/@" % sb::right(mpoint, -1));
                    sb::umount(part);
                    if (intrrpt)
                        goto exit;
                    ...
                    if (!sb::mount(part, "/.sbsystemcopy" % mpoint,
                        ...
                        dialogdev = part;
                        dialog
                            = ui->userdatafilescopy->isVisibleTo(ui->copypanel)
                                  ? 31
                                  : 51;
                        goto error;
                    }
                ...
                dialogdev = part;
                dialog = ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 31
                                                                           : 51;
                goto error;
            }
        }
    ...
    if (pname == tr("Currently running system")) {
        if (ui->usersettingscopy->isVisibleTo(ui->copypanel)) {
            ...
            case Qt::Unchecked:
                if (!sb::scopy(5, guname(), nullptr))
                    goto error;
                break;
            case Qt::PartiallyChecked:
                if (!sb::scopy(3, guname(), nullptr))
                    goto error;
                break;
            case Qt::Checked:
                if (!sb::scopy(4, guname(), nullptr))
                    goto error;
            }
        } else if (!sb::scopy(
                       nohmcpy ? 0 : ui->userdatafilescopy->isChecked() ? 1 : 2,
                       nullptr, nullptr))
            goto error;

        if (ui->userdatafilescopy->isVisibleTo(ui->copypanel)
            && sb::schdle[0] == "on"
            && !sb::crtfile("/.sbsystemcopy/etc/systemback.conf",
                            "storagedir=" % sb::sdir[0] % "\nliveworkdir="
                            % sb::sdir[2] % "\npointsnumber="
                            % QStr::number(sb::pnumber)
                            % "\ntimer=off\nschedule=" % sb::schdle[1] % ':'
                            % sb::schdle[2] % ':' % sb::schdle[3] % ':'
        ...
        if (!sb::isdir("/.systembacklivepoint")
            ...
            QFile::rename("/.systembacklivepoint",
                          "/.systembacklivepoint_" % sb::rndstr());
            if (!QDir().mkdir("/.systembacklivepoint"))
                goto error;
        }
        ...
        QStr mdev(
            sb::isfile("/cdrom/casper/filesystem.squashfs")
                ? "/cdrom/casper/filesystem.squashfs"
                : sb::isfile("/lib/live/mount/medium/live/filesystem.squashfs")
                      ? "/lib/live/mount/medium/live/filesystem.squashfs"
                      : "/live/image/live/filesystem.squashfs");

        if (!sb::mount(mdev, "/.systembacklivepoint", "loop")) {
            dialog = ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 53
                                                                       : 54;
            goto error;
        }
        ...
        if (ui->usersettingscopy->isVisibleTo(ui->copypanel)) {
            switch (ui->usersettingscopy->checkState()) {
            case Qt::Unchecked:
                if (!sb::scopy(5, guname(), "/.systembacklivepoint"))
                    goto error;
                break;
            case Qt::PartiallyChecked:
                if (!sb::scopy(3, guname(), "/.systembacklivepoint"))
                    goto error;
                break;
            case Qt::Checked:
                if (!sb::scopy(4, guname(), "/.systembacklivepoint"))
                    goto error;
            }
        } else if (!sb::scopy(
                       nohmcpy ? 0 : ui->userdatafilescopy->isChecked() ? 1 : 2,
                       nullptr, "/.systembacklivepoint"))
            goto error;
        ...
        if (ui->userdatafilescopy->isVisibleTo(ui->copypanel)) {
            ...
            if (cfg.contains("timer=on")) {
                ...
                if (!sb::crtfile("/.sbsystemcopy/etc/systemback.conf", nconf))
                    goto error;
            }
        }
    } else {
        if (ui->usersettingscopy->isVisibleTo(ui->copypanel)) {
            switch (ui->usersettingscopy->checkState()) {
            case Qt::Unchecked:
                if (!sb::scopy(5, guname(),
                               sb::sdir[1] % '/' % cpoint % '_' % pname))
                    goto error;
                break;
            case Qt::PartiallyChecked:
                if (!sb::scopy(3, guname(),
                               sb::sdir[1] % '/' % cpoint % '_' % pname))
                    goto error;
                break;
            case Qt::Checked:
                if (!sb::scopy(4, guname(),
                               sb::sdir[1] % '/' % cpoint % '_' % pname))
                    goto error;
            }
        } else if (!sb::scopy(
                       nohmcpy ? 0 : ui->userdatafilescopy->isChecked() ? 1 : 2,
                       nullptr, sb::sdir[1] % '/' % cpoint % '_' % pname))
            goto error;
    ...
    if (ui->usersettingscopy->isVisibleTo(ui->copypanel)) {
        ...
        if (guname() != ui->username->text()) {
            if (sb::isdir("/.sbsystemcopy/home/" % guname())
                && ((sb::exist("/.sbsystemcopy/home/" % ui->username->text())
                     && !QFile::rename(
                            "/.sbsystemcopy/home/" % ui->username->text(),
                            "/.sbsystemcopy/home/" % ui->username->text() % '_'
                            % sb::rndstr()))
                    || !QFile::rename("/.sbsystemcopy/home/" % guname(),
                                      "/.sbsystemcopy/home/"
                                      % ui->username->text())))
                goto error;

            {
                ...
                while (!file.atEnd()) {
                    ...
                    if (nline.startsWith(guname() % ':'))
                        nline.replace(0, guname().length(),
                                      ui->username->text());
                    nline = nline.replace(':' % guname() % ',',
                                          ':' % ui->username->text() % ',');
                    nline = nline.replace(',' % guname() % ',',
                                          ',' % ui->username->text() % ',');
                    if (nline.endsWith(':' % guname()))
                        nline.replace(nline.length() - guname().length(),
                                      guname().length(), ui->username->text());
                    nfile.append(nline % '\n');
                    if (intrrpt)
                        goto exit;
                }
            ...
            if (!sb::crtfile("/.sbsystemcopy/etc/group", nfile))
                goto error;
            nfile.clear();
            ...
            if (!sb::crtfile("/.sbsystemcopy/etc/gshadow", nfile))
                goto error;
            nfile.clear();

            if (sb::isfile("/.sbsystemcopy/etc/subuid")
                && sb::isfile("/.sbsystemcopy/etc/subgid")) {
                {
                ...
                if (!sb::crtfile("/.sbsystemcopy/etc/subuid", nfile))
                    goto error;
                nfile.clear();
                ...
                if (!sb::crtfile("/.sbsystemcopy/etc/subgid", nfile))
                    goto error;
                nfile.clear();
            }
        }
        ...
        if (!sb::crtfile("/.sbsystemcopy/etc/passwd", nfile))
            goto error;
        nfile.clear();
                ...
                if (cline.startsWith(guname() % ':')) {
                    ...
                    for (uchar a(0); a < uslst.count(); ++a)
                        ...
                        case 1:
                            nline.append(
                                QStr(crypt(
                                    ui->password1->text().toStdString().c_str(),
                                    QStr("$6$" % sb::rndstr(16))
                                        .toStdString()
                                        .c_str())) % ':');
                            break;
                    ...
                    for (uchar a(0); a < uslst.count(); ++a)
                        ...
                        case 1:
                            nline.append(
                                (ui->rootpassword1->text().isEmpty()
                                     ? "!"
                                     : QStr(crypt(ui->rootpassword1->text()
                                                      .toStdString()
                                                      .c_str(),
                                                  QStr("$6$" % sb::rndstr(16))
                                                      .toStdString()
        ...
        if (!sb::crtfile("/.sbsystemcopy/etc/shadow", nfile))
            goto error;
        nfile.clear();

        {
            ...
            if (ohname != ui->hostname->text()) {
                if (!sb::crtfile("/.sbsystemcopy/etc/hostname",
                                 ui->hostname->text() % '\n'))
                    goto error;
                ...
                while (!file.atEnd()) {
                    QStr nline(file.readLine().trimmed());
                    nline = nline.replace('\t' % ohname % '\t',
                                          '\t' % ui->hostname->text() % '\t');
                    if (nline.endsWith('\t' % ohname))
                        nline.replace(nline.length() - ohname.length(),
                                      ohname.length(), ui->hostname->text());
                    nfile.append(nline % '\n');
                    if (intrrpt)
                        goto exit;
                }

                if (!sb::crtfile("/.sbsystemcopy/etc/hosts", nfile))
                    goto error;
                nfile.clear();
                if (intrrpt)
                    goto exit;
        ...
        if (!sb::crtfile(
                "/.sbsystemcopy/deluser",
                "#!/bin/sh\nfor rmuser in $(grep :\\$6\\$* /etc/shadow | cut "
                "-d : "
                "-f 1)\ndo [ $rmuser = " % ui->username->text()
                % " ] || [ $rmuser = root ] || userdel $rmuser\ndone\n")
            || !QFile::setPermissions("/.sbsystemcopy/deluser",
                                      QFile::ExeOwner))
            goto error;
        if (intrrpt)
            goto exit;
        if (sb::exec("chroot /.sbsystemcopy /deluser") > 0)
            goto error;
    ...
    for (ushort a(0); a < ui->partitionsettings->rowCount(); ++a) {
        if (!ui->partitionsettings->item(a, 4)->text().isEmpty()) {
            if (ui->partitionsettings->item(a, 4)->text() == "/home"
                && !ui->partitionsettings->item(a, 3)->text().isEmpty()) {
                ...
                if (ui->partitionsettings->item(a, 4)->text() == "/") {
                    if (sb::like(
                            ...
                            { "_ext4_", "_ext3_", "_ext2_", "_jfs_", "_xfs_" }))
                        fstabtxt.append(
                            "# " % ui->partitionsettings->item(a, 4)->text()
                            % "\nUUID=" % uuid % "   /   "
                            % ui->partitionsettings->item(a, 5)->text()
                            % "   defaults,errors=remount-ro   0   1\n");
                    else if (ui->partitionsettings->item(a, 5)->text()
                             == "reiserfs")
                        fstabtxt.append(
                            "# " % ui->partitionsettings->item(a, 4)->text()
                            % "\nUUID=" % uuid % "   /   "
                    ...
                    fstabtxt.append(
                        ...
                        % sb::right(ui->partitionsettings->item(a, 4)->text(),
                                    -1) % "   0   2\n");
                else
                    fstabtxt.append(
                        "# " % ui->partitionsettings->item(a, 4)->text()
    ...
    if (!sb::crtfile("/.sbsystemcopy/etc/fstab", fstabtxt))
        goto error;
    fstabtxt.clear();

    if (ui->grubinstallcopy->isVisibleTo(ui->copypanel)
        && ui->grubinstallcopy->currentText() != tr("Disabled")) {
        if (intrrpt)
            goto exit;
        ...
        if (ui->grubinstallcopy->currentText() == "Auto") {
            ...
            if (!sb::crtfile("/.sbsystemcopy/grubinst",
                             "#!/bin/sh\nupdate-grub\ngrub-install --force "
                             % sb::left(mntdev, 8) % '\n'))
                goto error;
        } else if (!sb::crtfile("/.sbsystemcopy/grubinst",
                                "#!/bin/sh\nupdate-grub\ngrub-install --force "
                                % ui->grubinstallcopy->currentText() % '\n'))
            goto error;

        if (!QFile::setPermissions("/.sbsystemcopy/grubinst", QFile::ExeOwner))
            goto error;
        if (intrrpt)
            goto exit;

        if (sb::exec("chroot /.sbsystemcopy /grubinst") > 0) {
            dialog = ui->userdatafilescopy->isVisibleTo(ui->copypanel) ? 22
                                                                       : 34;
            goto error;
        }
...

    {
        ...
        while (!in.atEnd()) {
            QStr cline(in.readLine());
            if (cline.contains(" /.sbsystemcopy"))
                sb::umount(sb::left(cline, sb::instr(cline, " ") - 1));
        }
    }
...
start:
    ...
    if (intrrpt)
        goto error;
    ullong size(sb::fsize(
        sb::sdir[2] % '/'
        % sb::left(ui->livelist->currentItem()->text(),
                   sb::instr(ui->livelist->currentItem()->text(), " ") - 1)
        % ".sblive"));
    QStr lrdir;
    ...
    if (intrrpt
        || sb::exec("dd if=/usr/lib/syslinux/"
                    % QStr(sb::isfile("/usr/lib/syslinux/mbr.bin") ? nullptr
                                                                   : "mbr/")
                    % "mbr.bin of=" % ldev % " conv=notrunc bs=440 count=1") > 0
        || !sb::setpflag(ldev % '1', "boot") || !sb::setpflag(ldev % '1', "lba")
        || intrrpt || (sb::exist("/.sblivesystemwrite")
                       && (((sb::mcheck("/.sblivesystemwrite/sblive")
                             && !sb::umount("/.sblivesystemwrite/sblive"))
                            || (sb::mcheck("/.sblivesystemwrite/sbroot")
                                && !sb::umount("/.sblivesystemwrite/sbroot")))
                           || !sb::remove("/.sblivesystemwrite"))) || intrrpt
        || !QDir().mkdir("/.sblivesystemwrite")
        || !QDir().mkdir("/.sblivesystemwrite/sblive"))
        goto error;
    ...
    if (lrdir == "sbroot") {
        if (!QDir().mkdir("/.sblivesystemwrite/sbroot"))
            goto error;
    ...
    if (lrdir == "sblive") {
        if (sb::exec("tar -xf " % sb::sdir[2] % '/'
                     % sb::left(ui->livelist->currentItem()->text(),
                                sb::instr(ui->livelist->currentItem()->text(),
                                          " ") - 1)
                     % ".sblive -C /.sblivesystemwrite/sblive --no-same-owner "
            ...
            goto error;
                   ...
                   % sb::left(ui->livelist->currentItem()->text(),
                              sb::instr(ui->livelist->currentItem()->text(),
                                        " ") - 1)
                   % ".sblive -C /.sblivesystemwrite/sblive "
                     "--exclude=casper/filesystem.squashfs "
                     "--exclude=live/filesystem.squashfs --no-same-permissions "
                     "--no-same-owner") > 0) {
        dialog = 43;
        goto error;
    } else if (sb::exec(
                   ...
                   % sb::left(ui->livelist->currentItem()->text(),
                              sb::instr(ui->livelist->currentItem()->text(),
                                        " ") - 1)
                   % ".sblive -C /.sblivesystemwrite/sbroot --exclude=.disk "
                     "--exclude=boot --exclude=EFI --exclude=syslinux "
                     "--exclude=casper/initrd.gz --exclude=casper/vmlinuz "
                     "--exclude=live/initrd.gz --exclude=live/vmlinuz "
    ...
    prun = tr("Emptying cache");
    if (sb::exec("syslinux -ifd syslinux " % ldev % '1') > 0)
        goto error;
    sb::fssync();
    sb::crtfile("/proc/sys/vm/drop_caches", "3");
    sb::umount("/.sblivesystemwrite/sblive");
...
{
    if (ui->dialogcancel->isVisibleTo(ui->dialogpanel))
        ui->dialogcancel->hide();
    if (ui->dialogquestion->isVisibleTo(ui->dialogpanel))
        ui->dialogquestion->hide();
    if (ui->dialoginfo->isVisibleTo(ui->dialogpanel))
        ui->dialoginfo->hide();
    if (ui->dialogerror->isVisibleTo(ui->dialogpanel))
        ui->dialogerror->hide();
    if (ui->dialognumber->isVisibleTo(ui->dialogpanel))
        ui->dialognumber->hide();

    switch (dialog) {
    case 1:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("Another systemback process is currently "
                                   "running, please wait until it finishes."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 2:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("Unable to get exclusive lock!") % "<p>"
    ...
    case 3:
        ...
        ui->dialogtext->setText(
            tr("The specified name contain(s) unsupported character(s)!")
            % "<p>" % tr("Please enter a new name."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 4:
        ...
        ui->dialogtext->setText(
            tr("Restore the system files to the following restore point:")
            % "<p><b>" % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
        break;
    case 5:
        ui->dialogquestion->show();
        ui->dialogtext->setText(
            tr("Repair the system files with the following restore point:")
            % "<p><b>" % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
    ...
    case 6:
        ui->dialogquestion->show();
        ui->dialogtext->setText(
            tr("Repair the complete system with the following restore point:")
            % "<p><b>" % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
    ...
    case 7:
        ui->dialogquestion->show();
        ui->dialogtext->setText(tr("Restore the complete user(s) configuration "
                                   "files to the following restore point:")
                                % "<p><b>" % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
    ...
    case 8:
        ui->dialogquestion->show();
        ui->dialogtext->setText(
            tr("Restore the user(s) configuration files to the "
               "following restore point:") % "<p><b>" % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
        break;
    case 9:
        ui->dialoginfo->show();
        ui->dialogtext->setText(
            tr("User(s) configuration files full restoration are completed.")
            % "<p>"
            % tr("The X server will restart automatically within 30 seconds."));
        if (ui->dialogok->text() != tr("X restart"))
            ui->dialogok->setText(tr("X restart"));
        ui->dialogcancel->show();
    ...
    case 10:
        ui->dialoginfo->show();
        ui->dialogtext->setText(
            tr("User(s) configuration files restoration are completed.") % "<p>"
            % tr("The X server will restart automatically within 30 seconds."));
        if (ui->dialogok->text() != tr("X restart"))
            ui->dialogok->setText(tr("X restart"));
        ui->dialogcancel->show();
    ...
    case 11:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("System files repair are completed, but an error occurred while "
               "reinstalling GRUB!") % ' '
            % tr("System may not bootable! (In general, the different "
                 "architecture is causing the problem.)"));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 12:
    ...
    case 14:
        ui->dialogquestion->show();
        ui->dialogtext->setText(
            tr("Copy the system, using the following restore point:") % "<p><b>"
            % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
    ...
    case 15:
        ui->dialogquestion->show();
        ui->dialogtext->setText(
            tr("Install the system, using the following restore point:")
            % "<p><b>" % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
    ...
    case 16:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("Restore point creation is aborted!") % "<p>"
    ...
    case 18:
        ui->dialogquestion->show();
        ui->dialogtext->setText(
            tr("Restore complete system to the following restore point:")
            % "<p><b>" % pname);
        if (ui->dialogok->text() != tr("Start"))
            ui->dialogok->setText(tr("Start"));
        ui->dialogcancel->show();
    ...
    case 19:
        ui->dialoginfo->show();
        ui->dialogtext->setText(
            tr("System files restoration are completed.") % "<p>"
            % tr("The computer will restart automatically within 30 seconds."));
        if (ui->dialogok->text() != tr("Reboot"))
    ...
    case 20:
        ui->dialoginfo->show();
        ui->dialogtext->setText(
            tr("Full system restoration is completed.") % "<p>"
            % tr("The computer will restart automatically within 30 seconds."));
        if (ui->dialogok->text() != tr("Reboot"))
            ui->dialogok->setText(tr("Reboot"));
        ui->dialogcancel->show();
    ...
    case 21:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("System copy is aborted!") % "<p>"
            % tr("The specified partition(s) doesn't have enough free space to "
                 "copy "
                 "the system. The copied system will not function properly."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 22:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("System copy is completed, but an error "
                                   "occurred while installing GRUB!") % ' '
                                % tr("Need to manually install a bootloader."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 23:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("System restoration is aborted!") % "<p>"
            % tr("An error occurred while reinstalling GRUB."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 24:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("Full system repair is completed, but an error occurred while "
               "reinstalling GRUB!") % ' '
            % tr("System may not bootable! (In general, the different "
                 "architecture is causing the problem.)"));
        if (ui->dialogok->text() != "OK")
    ...
    case 26:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("Live system creation is aborted!") % "<p>"
            % tr("An error occurred while creating file system image."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 27:
        ...
        ui->dialogtext->setText(
            tr("Live system creation is aborted!") % "<p>"
            % tr("An error occurred while creating container file."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 28:
        ui->dialoginfo->show();
        ui->dialogtext->setText(
            tr("Live system creation is completed.") % "<p>"
            % tr("The created .sblive file can be written to pendrive."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 29:
    ...
    case 30:
        ui->dialogquestion->show();
        ui->dialogtext->setText(
            tr("Format the <dev>, and write the following Live system image:")
                .replace("<dev>",
                         " <b>"
                         % ui->livedevices->item(ui->livedevices->currentRow(),
                                                 0)->text() % "</b>") % "<p><b>"
            % sb::left(ui->livelist->currentItem()->text(),
                       sb::instr(ui->livelist->currentItem()->text(), " ") - 1)
            % "</b>");
    ...
    case 31:
        ...
        ui->dialogtext->setText(
            tr("System copy is aborted!") % "<p>"
            % tr("The specified partition couldn't be mounted.") % "<p><b>"
            % dialogdev);
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    ...
    case 34:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("System install is completed, but an error "
                                   "occurred while installing GRUB!") % ' '
                                % tr("Need to manually install a bootloader."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
    ...
    case 35:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("System installation is aborted!") % "<p>"
            % tr("The specified partition(s) doesn't have enough "
                 "free space to install the system. The "
                 "installed system will not function properly."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 36:
        ...
        ui->dialogtext->setText(
            tr("System copy is aborted!") % "<p>"
            % tr("The specified partition couldn't be formatted "
                 "(in use or unavailable).") % "<p><b>" % dialogdev);
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 37:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("An error occurred while reinstalling GRUB!")
                                % ' '
                                % tr("System may not bootable! (In general, "
    ...
    case 38:
        ...
        ui->dialogtext->setText(
            tr("Restore point creation is aborted!") % "<p>"
            % tr("There has been critical changes in the file "
    ...
    case 39:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("System copying is aborted!") % "<p>"
            % tr("There has been critical changes in the file "
                 "system during this operation."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
    ...
    case 42:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("Live write is aborted!") % "<p>"
            % tr("The selected partition doesn't have enough "
                 "space to write the Live system."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 43:
        ...
        ui->dialogtext->setText(
            tr("Live write is aborted!") % "<p>"
            % tr("An error occurred while unpacking Live system files."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 44:
    ...
    case 45:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("Live conversion is aborted!") % "<p>"
            % tr("An error occurred while renaming essential Live files."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 46:
        ...
        ui->dialogtext->setText(
            tr("Live conversion is aborted!") % "<p>"
            % tr("An error occurred while creating .iso image."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 47:
        ...
        ui->dialogtext->setText(
            tr("Live conversion is aborted!") % "<p>"
            % tr("An error occurred while reading .sblive image."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 48:
        ...
        ui->dialogtext->setText(
            tr("Live system creation is aborted!") % "<p>"
            % tr("An error occurred while creating new initramfs image."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 49:
    ...
    case 50:
        ui->dialogerror->show();
        ui->dialogtext->setText(
            tr("Restore point deletion is aborted!") % "<p>"
    ...
    case 53:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("System copy is aborted!") % "<p>"
                                % tr("The Live image couldn't be mounted."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 54:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("System installation is aborted!") % "<p>"
                                % tr("The Live image couldn't be mounted."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 55:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("System repair is aborted!") % "<p>"
                                % tr("The Live image couldn't be mounted."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    ...
    case 60:
        ui->dialogerror->show();
        ui->dialogtext->setText(tr("System restoration is aborted!") % "<p>"
                                % tr("There isn't enough free space."));
        if (ui->dialogok->text() != "OK")
            ui->dialogok->setText("OK");
        break;
    case 61:
    ...
    if (ui->statuspanel->isVisible())
        ui->statuspanel->hide();
    if (ui->schedulerpanel->isVisible())
        ui->schedulerpanel->hide();
    if (ui->dialogpanel->isHidden())
        ui->dialogpanel->show();
    ...
    if (width() != ui->dialogpanel->width()) {
        if (utimer->isActive() && !sstart)
            ...
            if (sstart && !ui->function3->text().contains(' '))
                ui->function3->setText("Systemback " % tr("scheduler"));
...
void systemback::setwontop(bool state)
{
    ...
    XSendEvent(dsply, XDefaultRootWindow(dsply), 0,
...
void systemback::windowmove(ushort nwidth, ushort nheight, bool fxdw)
{
    ...
    if (size() != QSize(wgeom[2], wgeom[3])) {
        ui->resizepanel->show();
        repaint();
        short wmvxy[2], rghtlmt(qApp->desktop()->width() - wgeom[2] - ss(30)),
            bttmlmt(qApp->desktop()->height() - wgeom[3] - ss(30));
...
void systemback::wmove()
{
    if (size() != qApp->desktop()->availableGeometry().size()) {
        if (ui->function1->isVisible())
            move(QCursor::pos().x() - ui->function1->MouseX,
                 QCursor::pos().y() - ui->function1->MouseY);
        else if (ui->function2->isVisible())
            move(QCursor::pos().x() - ui->function2->MouseX,
                 QCursor::pos().y() - ui->function2->MouseY);
        else if (ui->function3->isVisible())
            move(QCursor::pos().x() - ui->function3->MouseX,
                 QCursor::pos().y() - ui->function3->MouseY);
        else if (ui->function4->isVisible())
            move(QCursor::pos().x() - ui->function4->MouseX,
                 QCursor::pos().y() - ui->function4->MouseY);
    }
}
...
bool systemback::eventFilter(QObject *, QEvent *ev)
{
    ...
    case QEvent::WindowActivate:
        ...
        if (x() < 0 && x() > -width())
            wgeom[0] = ss(30);
                 ...
                 && x() < qApp->desktop()->width() + width())
            wgeom[0] = qApp->desktop()->width() - width() - ss(30);

        if (y() < 0 && y() > -height())
            wgeom[1] = ss(30);
        else if (y() > qApp->desktop()->height() - height()
                 && y() < qApp->desktop()->height() + height())
            wgeom[1] = qApp->desktop()->height() - height() - ss(30);
    ...
    case QEvent::Resize:
        ...
        if (ui->choosepanel->isVisible()) {
            ui->choosepanel->resize(width() - (sfctr == High ? 4 : ss(2)),
                                    height() - ss(25));
            ui->dirpath->resize(ui->choosepanel->width() - ss(40),
                                ui->dirpath->height());
            ui->dirrefresh->move(
                ui->choosepanel->width() - ui->dirrefresh->width(), 0);
            ui->dirchoose->resize(ui->choosepanel->width(),
                                  ui->choosepanel->height() - ss(80));
            ui->dirchooseok->move(ui->choosepanel->width() - ss(120),
                                  ui->choosepanel->height() - ss(40));
            ui->dirchoosecancel->move(ui->choosepanel->width() - ss(240),
                                      ui->choosepanel->height() - ss(40));
            ui->warning->move(ui->warning->x(),
                              ui->choosepanel->height() - ss(41));
            ui->chooseresize->move(
                ui->choosepanel->width() - ui->chooseresize->width(),
                ui->choosepanel->height() - ui->chooseresize->height());
            ...
            ui->copypanel->resize(width() - (sfctr == High ? 4 : ss(2)),
                                  height() - ss(25));
            ui->partitionsettingstext->resize(
                ui->copypanel->width(), ui->partitionsettingstext->height());
            ui->partitionsettings->resize(ui->copypanel->width() - ss(152),
                                          ui->copypanel->height() - ss(200));
            ui->partitionsettingspanel1->move(ui->partitionsettings->x()
                                              + ui->partitionsettings->width(),
                                              ui->partitionsettingspanel1->y());
            ui->partitionsettingspanel2->move(
                ui->partitionsettingspanel1->pos());
            ui->partitionsettingspanel3->move(
                ui->partitionsettingspanel1->pos());
            ui->partitionoptionstext->setGeometry(
                0, ui->copypanel->height() - ss(160), ui->copypanel->width(),
                ui->partitionoptionstext->height());
            ui->userdatafilescopy->move(ui->userdatafilescopy->x(),
                                        ui->copypanel->height() - ss(122)
                                        - (sfctr == High ? 1 : 2));
            ui->usersettingscopy->move(ui->usersettingscopy->x(),
                                       ui->userdatafilescopy->y());
            ui->grubinstalltext->move(ui->grubinstalltext->x(),
                                      ui->copypanel->height() - ss(96));
            ui->grubinstallcopy->move(ui->grubinstallcopy->x(),
                                      ui->grubinstalltext->y());
            ui->copyback->move(ui->copyback->x(),
                               ui->copypanel->height() - ss(48));
            ui->copynext->move(ui->copypanel->width() - ss(152),
                               ui->copyback->y());
            ui->copyresize->move(
                ui->copypanel->width() - ui->copyresize->width(),
                ui->copypanel->height() - ui->copyresize->height());
            ui->copycover->resize(ui->copypanel->width() - ss(10),
                                  ui->copypanel->height() - ss(10));
            ...
            ui->excludepanel->resize(width() - (sfctr == High ? 4 : ss(2)),
                                     height() - ss(25));
            ui->itemstext->resize(ui->excludepanel->width() / 2 - ss(44)
                                  + (sfctr == High ? 1 : 0),
                                  ui->itemstext->height());
            ui->excludedtext->setGeometry(
                ui->excludepanel->width() / 2 + ss(36), ui->excludedtext->y(),
                ui->itemstext->width(), ui->excludedtext->height());
            ui->itemslist->resize(ui->itemstext->width(),
                                  ui->excludepanel->height() - ss(160));
            ui->excludedlist->setGeometry(
                ui->excludepanel->width() / 2 + ss(36), ui->excludedlist->y(),
                ui->itemslist->width(), ui->itemslist->height());
            ui->additem->move(ui->excludepanel->width() / 2 - ss(24),
                              ui->itemslist->height() / 2 + ss(36));
            ui->removeitem->move(ui->additem->x(),
                                 ui->itemslist->height() / 2 + ss(108));
            ui->excludeback->move(ui->excludeback->x(),
                                  ui->excludepanel->height() - ss(48));
            ui->kendektext->move(ui->excludepanel->width() - ss(306),
                                 ui->excludepanel->height() - ss(24));
            ui->excluderesize->move(
                ui->excludepanel->width() - ui->excluderesize->width(),
                ui->excludepanel->height() - ui->excluderesize->height());
...
void systemback::keyPressEvent(QKeyEvent *ev)
{
    ...
    case Qt::Key_Return: {
        ...
        if (ui->sbpanel->isVisible()) {
            ...
            ui->dirchoose->currentItem()->setExpanded(
                !ui->dirchoose->currentItem()->isExpanded());
        else if (ui->repairmountpoint->hasFocus()) {
            if (ui->repairmount->isEnabled()) {
                on_repairmount_clicked();
                if (!ui->repairmountpoint->hasFocus())
                    ui->repairmountpoint->setFocus();
            }
            ...
            if (ui->mountpoint->hasFocus()) {
                if (ui->changepartition->isEnabled())
                    on_changepartition_clicked();
            } else if (ui->partitionsize->hasFocus()) {
                if (ui->newpartition->isEnabled())
    ...
    case Qt::Key_F5:
        if (ui->sbpanel->isVisible()) {
            if (ui->point1->hasFocus()) {
                if (ui->point1->text() != sb::pnames[0])
                    ui->point1->setText(sb::pnames[0]);
            } else if (ui->point2->hasFocus()) {
                if (ui->point2->text() != sb::pnames[1])
                    ui->point2->setText(sb::pnames[1]);
            } else if (ui->point3->hasFocus()) {
                if (ui->point3->text() != sb::pnames[2])
                    ui->point3->setText(sb::pnames[2]);
            } else if (ui->point4->hasFocus()) {
                if (ui->point4->text() != sb::pnames[3])
                    ui->point4->setText(sb::pnames[3]);
            } else if (ui->point5->hasFocus()) {
                if (ui->point5->text() != sb::pnames[4])
                    ui->point5->setText(sb::pnames[4]);
            } else if (ui->point6->hasFocus()) {
                if (ui->point6->text() != sb::pnames[5])
                    ui->point6->setText(sb::pnames[5]);
            } else if (ui->point7->hasFocus()) {
                if (ui->point7->text() != sb::pnames[6])
                    ui->point7->setText(sb::pnames[6]);
            } else if (ui->point8->hasFocus()) {
                if (ui->point8->text() != sb::pnames[7])
                    ui->point8->setText(sb::pnames[7]);
            } else if (ui->point9->hasFocus()) {
                if (ui->point9->text() != sb::pnames[8])
                    ui->point9->setText(sb::pnames[8]);
            } else if (ui->point10->hasFocus()) {
                if (ui->point10->text() != sb::pnames[9])
                    ui->point10->setText(sb::pnames[9]);
            } else if (ui->point11->hasFocus()) {
                if (ui->point11->text() != sb::pnames[10])
                    ui->point11->setText(sb::pnames[10]);
            } else if (ui->point12->hasFocus()) {
                if (ui->point12->text() != sb::pnames[11])
                    ui->point12->setText(sb::pnames[11]);
            } else if (ui->point13->hasFocus()) {
                if (ui->point13->text() != sb::pnames[12])
                    ui->point13->setText(sb::pnames[12]);
            } else if (ui->point14->hasFocus()) {
                if (ui->point14->text() != sb::pnames[13])
                    ui->point14->setText(sb::pnames[13]);
            } else if (ui->point15->hasFocus() && ui->point15->text()
                                                  != sb::pnames[14])
                ui->point15->setText(sb::pnames[14]);
        } else if (ui->partitionsettings->hasFocus()
                   || ui->mountpoint->hasFocus()
                   || ui->partitionsize->hasFocus()) {
    ...
    case Qt::Key_Delete:
        if (ui->partitionsettings->hasFocus()) {
            if (ui->umountdelete->isEnabled() && !ui->copycover->isVisible()
                && ui->umountdelete->text() == tr("Umount"))
                on_umountdelete_clicked();
        } else if (ui->livelist->hasFocus()) {
            if (ui->livedelete->isEnabled())
                on_livedelete_clicked();
        } else if (ui->itemslist->hasFocus())
            on_additem_clicked();
        else if (ui->excludedlist->hasFocus())
            on_removeitem_clicked();
...
void systemback::on_admins_currentIndexChanged(const QStr &arg1)
{
        ...
        if (file.open(QIODevice::ReadOnly))
            while (!file.atEnd()) {
                ...
                if (cline.startsWith(arg1 % ":")) {
                    hash = sb::mid(cline, arg1.length() + 2,
                                   sb::instr(cline, ":", arg1.length() + 2)
                                   - arg1.length() - 2);
                    break;
                }
            }
    ...
    if (ui->adminpassword->text().length() > 0)
        ui->adminpassword->clear();

    if (!hash.isEmpty() && QStr(crypt("", hash.toStdString().c_str()))
                           == hash) {
...
void systemback::on_adminpassword_textChanged(const QStr &arg1)
{
    ...
    if (arg1.isEmpty()) {
        if (ui->adminpassworderror->isVisible())
            ui->adminpassworderror->hide();
    } else if (hash.isEmpty()) {
        if (ui->adminpassworderror->isHidden())
            ui->adminpassworderror->show();
    } else if (QStr(crypt(ipasswd.toStdString().c_str(),
                          hash.toStdString().c_str())) == hash) {
        sb::delay(300);
...
void systemback::on_startcancel_clicked() { close(); }
...
void systemback::on_dialogcancel_clicked()
{
    if (dialog != 30) {
        if (dlgtimer->isActive()) {
            dlgtimer->stop();
            if (ui->dialognumber->text() != "30s")
                ui->dialognumber->setText("30s");
        }
...
void systemback::on_pnumber3_clicked()
{
    ...
    if (ui->point3->isEnabled() && ui->point3->styleSheet().isEmpty())
        ui->point3->setStyleSheet("background-color: rgb(255, 103, 103)");

    if (ui->point4->isEnabled()) {
        if (ui->point4->styleSheet().isEmpty())
            ui->point4->setStyleSheet("background-color: rgb(255, 103, 103)");
    } else if (ui->point4->text() == tr("empty"))
        ui->point4->setText(tr("not used"));

    if (ui->point5->isEnabled()) {
        if (ui->point5->styleSheet().isEmpty())
            ui->point5->setStyleSheet("background-color: rgb(255, 103, 103)");
    } else if (ui->point5->text() == tr("empty"))
        ui->point5->setText(tr("not used"));

    if (ui->point6->isEnabled()) {
        if (ui->point6->styleSheet().isEmpty())
            ui->point6->setStyleSheet("background-color: rgb(255, 103, 103)");
    } else if (ui->point6->text() == tr("empty"))
        ui->point6->setText(tr("not used"));

    if (ui->point7->isEnabled()) {
        if (ui->point7->styleSheet().isEmpty())
            ui->point7->setStyleSheet("background-color: rgb(255, 103, 103)");
    } else if (ui->point7->text() == tr("empty"))
        ui->point7->setText(tr("not used"));

    if (ui->point8->isEnabled()) {
        if (ui->point8->styleSheet().isEmpty())
            ui->point8->setStyleSheet("background-color: rgb(255, 103, 103)");
    } else if (ui->point8->text() == tr("empty"))
        ui->point8->setText(tr("not used"));

    if (ui->point9->isEnabled()) {
        if (ui->point9->styleSheet().isEmpty())
            ui->point9->setStyleSheet("background-color: rgb(255, 103, 103)");
    } else if (ui->point9->text() == tr("empty"))
        ui->point9->setText(tr("not used"));

    if (ui->point10->isEnabled()) {
        if (ui->point10->styleSheet().isEmpty())
            ui->point10->setStyleSheet("background-color: rgb(255, 103, 103)");
    } else if (ui->point10->text() == tr("empty"))
        ui->point10->setText(tr("not used"));
        ...
        if (arg1.isEmpty()) {
            ...
            if (arg1 != sb::pnames[0]) {
                ...
                if (!ui->pointrename->isEnabled())
                    ui->pointrename->setEnabled(true);
            } else if (ui->pointpipe1->isChecked())
                ui->pointpipe1->click();

            if (!ui->pointpipe1->isEnabled())
                ui->pointpipe1->setEnabled(true);
        }
...
void systemback::on_point2_textChanged(const QStr &arg1)
{
    if (ui->point2->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe2->isEnabled())
                ui->pointpipe2->setEnabled(true);
        }
...
void systemback::on_point3_textChanged(const QStr &arg1)
{
    if (ui->point3->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe3->isEnabled())
                ui->pointpipe3->setEnabled(true);
        }
...
void systemback::on_point4_textChanged(const QStr &arg1)
{
    if (ui->point4->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe4->isEnabled())
                ui->pointpipe4->setEnabled(true);
        }
...
void systemback::on_point5_textChanged(const QStr &arg1)
{
    if (ui->point5->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe5->isEnabled())
                ui->pointpipe5->setEnabled(true);
        }
...
void systemback::on_point6_textChanged(const QStr &arg1)
{
    if (ui->point6->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe6->isEnabled())
                ui->pointpipe6->setEnabled(true);
        }
...
void systemback::on_point7_textChanged(const QStr &arg1)
{
    if (ui->point7->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe7->isEnabled())
                ui->pointpipe7->setEnabled(true);
        }
...
void systemback::on_point8_textChanged(const QStr &arg1)
{
    if (ui->point8->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe8->isEnabled())
                ui->pointpipe8->setEnabled(true);
        }
...
void systemback::on_point9_textChanged(const QStr &arg1)
{
    if (ui->point9->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe9->isEnabled())
                ui->pointpipe9->setEnabled(true);
        }
...
void systemback::on_point10_textChanged(const QStr &arg1)
{
    if (ui->point10->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe10->isEnabled())
                ui->pointpipe10->setEnabled(true);
        }
...
void systemback::on_point11_textChanged(const QStr &arg1)
{
    if (ui->point11->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe11->isEnabled())
                ui->pointpipe11->setEnabled(true);
        }
...
void systemback::on_point12_textChanged(const QStr &arg1)
{
    if (ui->point12->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe12->isEnabled())
                ui->pointpipe12->setEnabled(true);
        }
...
void systemback::on_point13_textChanged(const QStr &arg1)
{
    if (ui->point13->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe13->isEnabled())
                ui->pointpipe13->setEnabled(true);
        }
...
void systemback::on_point14_textChanged(const QStr &arg1)
{
    if (ui->point14->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe14->isEnabled())
                ui->pointpipe14->setEnabled(true);
        }
...
void systemback::on_point15_textChanged(const QStr &arg1)
{
    if (ui->point15->isEnabled()) {
        if (arg1.isEmpty()) {
            ...
            if (!ui->pointpipe15->isEnabled())
                ui->pointpipe15->setEnabled(true);
        }
...
void systemback::on_restoremenu_clicked()
{
    if (sb::execsrch("update-grub2", sb::sdir[1] % '/' % cpoint % '_' % pname)
        && sb::isfile(sb::sdir[1] % '/' % cpoint % '_' % pname
                      % "/var/lib/dpkg/info/grub-" % grub % ".list")) {
    ...
    if (ui->includeusers->count() > 0)
        ui->includeusers->clear();
    ui->includeusers->addItems({ tr("Everyone"), "root" });
    if (!ui->restorenext->isEnabled())
        ui->restorenext->setEnabled(true);
    ui->sbpanel->hide();
...
void systemback::on_copymenu_clicked()
{
    ...
    } else if (sb::execsrch("update-grub2")
               && sb::isfile("/var/lib/dpkg/info/grub-" % grub % ".list")) {
        if (ui->grubinstallcopydisable->isVisibleTo(ui->copypanel)) {
            ui->grubinstallcopydisable->hide();
            ui->grubinstallcopy->show();
        }
    ...
    if (sb::like(pname, { '_' % tr("Currently running system") % '_',
                          '_' % tr("Live image") % '_' })) {
        if (!ui->userdatafilescopy->isEnabled())
            ui->userdatafilescopy->setEnabled(true);
    } else if (ui->userdatafilescopy->isEnabled()) {
        ui->userdatafilescopy->setDisabled(true);
        if (ui->userdatafilescopy->isChecked())
            ui->userdatafilescopy->setChecked(false);
    }

    short nwidth(ss(154) + ui->partitionsettings->width()
                 - ui->partitionsettings->contentsRect().width()
                 + ui->partitionsettings->columnWidth(0)
                 + ui->partitionsettings->columnWidth(1)
                 + ui->partitionsettings->columnWidth(2)
                 + ui->partitionsettings->columnWidth(3)
                 + ui->partitionsettings->columnWidth(4)
                 + ui->partitionsettings->columnWidth(5)
                 + ui->partitionsettings->columnWidth(6));
    ui->sbpanel->hide();
    ui->copypanel->show();
    ...
    setMaximumSize(qApp->desktop()->availableGeometry().width() - ss(60),
                   qApp->desktop()->availableGeometry().height() - ss(60));

    if (ui->partitionsettings->currentItem()) {
        if (sb::mcheck("/.sbsystemcopy/") || sb::mcheck("/.sblivesystemwrite/"))
            on_partitionrefresh_clicked();
        else if (ui->partitionsettings->item(
                                            ui->partitionsettings->currentRow(),
                                            3)->text() == "/home"
                 && !ui->mountpoint->isEnabled())
...
void systemback::on_livecreatemenu_clicked()
{
    ...
    if (sb::isdir(sb::sdir[2]))
        for (cQStr &item :
             QDir(sb::sdir[2]).entryList(QDir::Files | QDir::Hidden)) {
            if (item.endsWith(".sblive") && !item.contains(' ')
                && !sb::islink(sb::sdir[2] % '/' % item)
                && sb::fsize(sb::sdir[2] % '/' % item) > 0) {
                QLWI *lwi(new QLWI(
                    sb::left(item, -7) % " ("
                    % QStr::number(qRound64(sb::fsize(sb::sdir[2] % '/' % item)
                                            * 100.0 / 1024.0 / 1024.0 / 1024.0)
                                   / 100.0) % " GiB, "
                    % (sb::stype(sb::sdir[2] % '/' % sb::left(item, -6) % "iso")
                           == sb::Isfile
                           && sb::fsize(sb::sdir[2] % '/' % sb::left(item, -6)
                                        % "iso") > 0
                           ? "sblive+iso"
                           : "sblive") % ')'));
...
void systemback::on_systemupgrade_clicked()
{
    ...
    sb::exec(
        "xterm +sb -bg grey85 -fg grey25 -fa a -fs 9 -geometry 80x24+80+70 "
        "-n \"System upgrade\" -T \"System upgrade\" -cr grey40 -selbg "
        "grey86 -bw 0 -bc -bcf 500 -bcn 500 -e sbsysupgrade");
...
void systemback::on_partitionrefresh_clicked()
{
    ...
    if (ui->partitionsettings->rowCount() > 0)
        ui->partitionsettings->clearContents();
    if (ui->repairpartition->count() > 0)
        ui->repairpartition->clear();
    ...
    schar sn(-1);
    QStr mntlst(sb::fload("/proc/self/mounts")),
        swplst(sb::fload("/proc/swaps"));
    ...
    for (cQStr &cdts : plst) {
        ...
        if (path.length() == 8) {
            ...
            if (sn > 0)
                ui->partitionsettings->setRowHeight(sn, ss(25));
            ...
            if (bsize < 1073741824)
                size->setText(QStr::number((bsize * 10 / 1048576 + 5) / 10)
                              % " MiB");
            else if (bsize < 1073741824000)
                size->setText(QStr::number(qRound64(bsize * 100.0 / 1024.0
                                                    / 1024.0 / 1024.0) / 100.0)
                              % " GiB");
            else
                size->setText(
                    QStr::number(qRound64(bsize * 100.0 / 1024.0 / 1024.0
                                          / 1024.0 / 1024.0) / 100.0) % " TiB");
            ...
            case sb::Logical: {
                ...
                if (uuid.isEmpty()) {
                    if (QStr('\n' % mntlst).contains('\n' % path % ' ')) {
                        QStr mnt(
                            sb::right(mntlst, -sb::instr(mntlst, path % ' ')));
                        short spc(sb::instr(mnt, " "));
                        mpt->setText(sb::mid(mnt, spc + 1,
                                             sb::instr(mnt, " ", spc + 1) - spc
                                             - 1).replace("\\040", " "));
                    }
                } else {
                    if (QStr('\n' % mntlst).contains('\n' % path % ' ')) {
                        if (QStr('\n' % mntlst).count('\n' % path % ' ') > 1
                            ...
                            QStr mnt(sb::right(mntlst,
                                               -sb::instr(mntlst, path % ' ')));
                            short spc(sb::instr(mnt, " "));
                            mpt->setText(sb::mid(mnt, spc + 1,
                                                 sb::instr(mnt, " ", spc + 1)
                                                 - spc
                        ...
                        if (QStr('\n' % mntlst)
                            ...
                            QStr mnt(sb::right(
                                mntlst, -sb::instr(mntlst, "/dev/disk/by-uuid/"
                                                           % uuid % ' ')));
                            short spc(sb::instr(mnt, " "));
                            mpt->setText(sb::mid(mnt, spc + 1,
                                                 sb::instr(mnt, " ", spc + 1)
...
void systemback::on_umountdelete_clicked()
{
    ...
    if (ui->umountdelete->text() == tr("Umount")) {
        if (ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)
                ->text() != "SWAP") {
            QStr mnts[2];
            mnts[0] = sb::fload("/proc/self/mounts");
            QTS in(&mnts[0], QIODevice::ReadOnly);
            ...
            while (!in.atEnd()) {
                ...
                if (sb::like(
                        cline,
                        { "* "
                          % ui->partitionsettings
                                ->item(ui->partitionsettings->currentRow(), 3)
                                ->text()
                                .replace(" ", "\\040") % " *",
                          "* "
                          % ui->partitionsettings
            ...
            for (ushort a(0); a < ui->partitionsettings->rowCount(); ++a) {
                ...
                if (!mpt.isEmpty() && mpt != "SWAP"
                    && !mnts[0].contains(' ' % mpt.replace(" ", "\\040") % ' '))
            ...
            sb::fssync();
                       ...
                       '\n' % ui->partitionsettings
                                  ->item(ui->partitionsettings->currentRow(), 0)
                                  ->text() % ' ')
                   || swapoff(ui->partitionsettings
                                  ->item(ui->partitionsettings->currentRow(), 0)
                                  ->text()
                                  .toStdString()
                                  .c_str()) == 0)
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)
                ->setText(nullptr);
        ...
        if (ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)
            ...
            if (minside(ui->copypanel->pos()
                        + ui->partitionsettingspanel1->pos()
                        + ui->umountdelete->pos(),
                        ui->umountdelete->geometry()))
                ui->umountdelete->setDisabled(true);
            if (!ui->mountpoint->isEnabled())
                ui->mountpoint->setEnabled(true);
        ...
        sb::delpart(
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 0)
                ->text());
        on_partitionrefresh2_clicked();
    }
                        ...
                        if (sb::like(
                                ...
                                { "* " % ui->partitionsettings->item(a, 3)
                                  ...
                                  "* " % ui->partitionsettings->item(a, 3)
                                             ...
                                             .replace(" ", "\\040") % "/*" }))
                            sb::umount(
                                sb::left(cline, sb::instr(cline, " ") - 1));
                    }
                }
            }
...
void systemback::on_pointpipe1_clicked()
{
    ...
    if (ui->pointpipe1->isChecked()) {
        ...
        if (ui->point1->text() != sb::pnames[0])
            rnmenbl = true;
    } else if (ui->point1->isEnabled() && ui->point1->text() != sb::pnames[0]
               && !ui->point1->text().isEmpty())
        ui->point1->setText(sb::pnames[0]);
    ...
    if (ui->pointpipe2->isChecked()) {
        ...
        if (ui->point2->text() != sb::pnames[1] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point2->isEnabled() && ui->point2->text() != sb::pnames[1]
               && !ui->point2->text().isEmpty())
        ui->point2->setText(sb::pnames[1]);
    ...
    if (ui->pointpipe3->isChecked()) {
        ...
        if (ui->point3->text() != sb::pnames[2] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point3->isEnabled() && ui->point3->text() != sb::pnames[2]
               && !ui->point3->text().isEmpty())
        ui->point3->setText(sb::pnames[2]);
    ...
    if (ui->pointpipe4->isChecked()) {
        ...
        if (ui->point4->text() != sb::pnames[3] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point4->isEnabled() && ui->point4->text() != sb::pnames[3]
               && !ui->point4->text().isEmpty())
        ui->point4->setText(sb::pnames[3]);
    ...
    if (ui->pointpipe5->isChecked()) {
        ...
        if (ui->point5->text() != sb::pnames[4] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point5->isEnabled() && ui->point5->text() != sb::pnames[4]
               && !ui->point5->text().isEmpty())
        ui->point5->setText(sb::pnames[4]);
    ...
    if (ui->pointpipe6->isChecked()) {
        ...
        if (ui->point6->text() != sb::pnames[5] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point6->isEnabled() && ui->point6->text() != sb::pnames[5]
               && !ui->point6->text().isEmpty())
        ui->point6->setText(sb::pnames[5]);
    ...
    if (ui->pointpipe7->isChecked()) {
        ...
        if (ui->point7->text() != sb::pnames[6] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point7->isEnabled() && ui->point7->text() != sb::pnames[6]
               && !ui->point7->text().isEmpty())
        ui->point7->setText(sb::pnames[6]);
    ...
    if (ui->pointpipe8->isChecked()) {
        ...
        if (ui->point8->text() != sb::pnames[7] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point8->isEnabled() && ui->point8->text() != sb::pnames[7]
               && !ui->point8->text().isEmpty())
        ui->point8->setText(sb::pnames[7]);
    ...
    if (ui->pointpipe9->isChecked()) {
        ...
        if (ui->point9->text() != sb::pnames[8] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point9->isEnabled() && ui->point9->text() != sb::pnames[8]
               && !ui->point9->text().isEmpty())
        ui->point9->setText(sb::pnames[8]);
    ...
    if (ui->pointpipe10->isChecked()) {
        ...
        if (ui->point10->text() != sb::pnames[9] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point10->isEnabled() && ui->point10->text() != sb::pnames[9]
               && !ui->point10->text().isEmpty())
        ui->point10->setText(sb::pnames[9]);
    ...
    if (ui->pointpipe11->isChecked()) {
        ...
        if (ui->point11->text() != sb::pnames[10] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point11->isEnabled() && ui->point11->text() != sb::pnames[10]
               && !ui->point11->text().isEmpty())
        ui->point11->setText(sb::pnames[10]);
    ...
    if (ui->pointpipe12->isChecked()) {
        ...
        if (ui->point12->text() != sb::pnames[11] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point12->isEnabled() && ui->point12->text() != sb::pnames[11]
               && !ui->point12->text().isEmpty())
        ui->point12->setText(sb::pnames[11]);
    ...
    if (ui->pointpipe13->isChecked()) {
        ...
        if (ui->point13->text() != sb::pnames[12] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point13->isEnabled() && ui->point13->text() != sb::pnames[12]
               && !ui->point13->text().isEmpty())
        ui->point13->setText(sb::pnames[12]);
    ...
    if (ui->pointpipe14->isChecked()) {
        ...
        if (ui->point14->text() != sb::pnames[13] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point14->isEnabled() && ui->point14->text() != sb::pnames[13]
               && !ui->point14->text().isEmpty())
        ui->point14->setText(sb::pnames[13]);
    ...
    if (ui->pointpipe15->isChecked()) {
        ...
        if (ui->point15->text() != sb::pnames[14] && !rnmenbl)
            rnmenbl = true;
    } else if (ui->point15->isEnabled() && ui->point15->text() != sb::pnames[14]
               && !ui->point15->text().isEmpty())
        ui->point15->setText(sb::pnames[14]);

    if (ppipe == 0) {
        if (ui->restoremenu->isEnabled())
            ui->restoremenu->setDisabled(true);
        ...
        if (ui->pointhighlight->isEnabled())
            ui->pointhighlight->setDisabled(true);
        if (!ui->repairmenu->isEnabled())
            ui->repairmenu->setEnabled(true);
        ...
        if (!sb::isfile("/cdrom/casper/filesystem.squashfs")
            ...
            if (!ui->newrestorepoint->isEnabled())
                ui->newrestorepoint->setEnabled(true);
            if (!ui->livecreatemenu->isEnabled())
        ...
        if (!rnmenbl && ui->pointrename->isEnabled())
            ui->pointrename->setDisabled(true);
        ...
        if (ppipe == 1) {
            ...
            if (ui->pointpipe11->isChecked() || ui->pointpipe12->isChecked()
                || ui->pointpipe13->isChecked() || ui->pointpipe14->isChecked()
                || ui->pointpipe15->isChecked()) {
                if (ui->pointhighlight->isEnabled())
                    ui->pointhighlight->setDisabled(true);
            } else if (!ui->point15->isEnabled()
                       && !ui->pointhighlight->isEnabled())
            ...
            if (!ui->restoremenu->isEnabled()
                ...
                && !sb::isfile(
                       "/lib/live/mount/medium/live/filesystem.squashfs"))
                ui->restoremenu->setEnabled(true);
            ...
            if (ui->livecreatemenu->isEnabled())
                ui->livecreatemenu->setDisabled(true);
            if (!ui->repairmenu->isEnabled())
                ui->repairmenu->setEnabled(true);
        } else {
            if (ui->restoremenu->isEnabled())
...
void systemback::on_pointpipe2_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe3_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe4_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe5_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe6_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe7_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe8_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe9_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe10_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe11_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe12_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe13_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe14_clicked() { on_pointpipe1_clicked(); }
void systemback::on_pointpipe15_clicked() { on_pointpipe1_clicked(); }

void systemback::on_livedevicesrefresh_clicked()
{
    busy();
    ui->livecreatecover->show();
    if (ui->livedevices->rowCount() > 0)
        ui->livedevices->clearContents();
    QSL dlst;
    ...
    for (cQStr &cdts : dlst) {
        ...
        if (bsize < 1073741824)
            ...
            size->setText(QStr::number(qRound64(bsize * 100.0 / 1024.0 / 1024.0
                                                / 1024.0) / 100.0) % " GiB");
        else
            size->setText(QStr::number(qRound64(bsize * 100.0 / 1024.0 / 1024.0
                                                / 1024.0 / 1024.0) / 100.0)
    ...
    if (ui->livedevices->columnWidth(0) + ui->livedevices->columnWidth(1)
        + ui->livedevices->columnWidth(2) + ui->livedevices->columnWidth(3)
        > ui->livedevices->contentsRect().width())
        ui->livedevices->setColumnWidth(2,
                                        ui->livedevices->contentsRect().width()
                                        - ui->livedevices->columnWidth(0)
                                        - ui->livedevices->columnWidth(1)
...
{
    busy();
    if (ui->itemslist->topLevelItemCount() > 0)
        ui->itemslist->clear();
    if (ui->excludedlist->count() > 0)
        ui->excludedlist->clear();
        ...
        if (file.open(QIODevice::ReadOnly)) {
            while (!file.atEnd()) {
                ...
                if (usr.contains(":/home/")) {
                    ...
                    if (sb::isdir("/home/" % usr))
                        for (cQStr &item : QDir("/home/" % usr).entryList(
                                 QDir::AllEntries | QDir::Hidden
                                 | QDir::NoDotAndDotDot))
                            if (((item.startsWith('.')
                                  && ui->pointexclude->isChecked())
                                 || (!item.startsWith('.')
                                     && ui->liveexclude->isChecked()))
                                && ui->excludedlist->findItems(item,
                                                               Qt::MatchExactly)
                                       .isEmpty()
                                && !sb::like(item, { "_.gvfs_", "_.Xauthority_",
                                                     "_.ICEauthority_" })) {
                                if (ui->itemslist->findItems(item,
                                                             Qt::MatchExactly)
                                        .isEmpty()) {
                                    QTrWI *twi(new QTrWI);
                                    twi->setText(0, item);

                                    if (sb::access("/home/" % usr % '/' % item)
                                        && sb::stype("/home/" % usr % '/'
                                                     % item) == sb::Isdir) {
                                        twi->setIcon(
                                            0,
                                        ...
                                        ui->itemslist->addTopLevelItem(twi);
                                } else if (sb::access("/home/" % usr % '/'
                                                      % item)) {
                                    if (sb::stype("/home/" % usr % '/' % item)
                                        == sb::Isdir) {
                                        QTrWI *ctwi(
                                            ui->itemslist->findItems(
                                                               item,
                                                               Qt::MatchExactly)
                                                .at(0));
                                        if (ctwi->icon(0).isNull())
                                            ctwi->setIcon(
                                                0, QIcon(":pictures/dir.png"));
                                        QSL sdlst(
                                            QDir("/home/" % usr % '/' % item)
                                                .entryList(
                                                    QDir::AllEntries
    ...
    for (cQStr &item : QDir("/root").entryList(QDir::AllEntries | QDir::Hidden
                                               | QDir::NoDotAndDotDot))
        if (((item.startsWith('.') && ui->pointexclude->isChecked())
             || (!item.startsWith('.') && ui->liveexclude->isChecked()))
            && ui->excludedlist->findItems(item, Qt::MatchExactly).isEmpty()
            && !sb::like(item,
                         { "_.gvfs_", "_.Xauthority_", "_.ICEauthority_" })) {
            if (ui->itemslist->findItems(item, Qt::MatchExactly).isEmpty()) {
                QTrWI *twi(new QTrWI);
                twi->setText(0, item);

                if (sb::access("/root/" % item) && sb::stype("/root/" % item)
                                                   == sb::Isdir) {
                    twi->setIcon(0, QIcon(QPixmap(":pictures/dir.png").scaled(
                                        ss(12), ss(9), Qt::IgnoreAspectRatio,
                                        Qt::SmoothTransformation)));
                    ui->itemslist->addTopLevelItem(twi);

                    for (cQStr &sitem : QDir("/root/" % item).entryList(
                             QDir::AllEntries | QDir::Hidden
                             | QDir::NoDotAndDotDot))
                        if (ui->excludedlist->findItems(item % '/' % sitem,
                                                        Qt::MatchExactly)
                                .isEmpty() && item % '/' % sitem
                                              != ".cache/gvfs") {
                            QTrWI *ctwi(new QTrWI);
                ...
                if (sb::stype("/root/" % item) == sb::Isdir) {
                    QTrWI *ctwi(
                        ui->itemslist->findItems(item, Qt::MatchExactly).at(0));
                    if (ctwi->icon(0).isNull())
                        ctwi->setIcon(0,
                                      QIcon(QPixmap(":pictures/dir.png").scaled(
                                          ss(12), ss(9), Qt::IgnoreAspectRatio,
                                          Qt::SmoothTransformation)));
                    QSL itmlst;
                    for (ushort a(0); a < ctwi->childCount(); ++a)
                        itmlst.append(ctwi->child(a)->text(0));
...
void systemback::on_liveexclude_clicked() { on_pointexclude_clicked(); }
...
void systemback::on_dialogok_clicked()
{
    if (ui->dialogok->text() == "OK") {
        if (dialog == 23) {
            ...
            for (cQStr &item : QDir(sb::sdir[1]).entryList(
            ...
            close();
        else if (sb::ilike(dialog, { 3, 21, 26, 27, 28, 29, 31, 35, 36, 39, 40,
            ...
            if (ui->copypanel->isVisible()) {
                ...
                short nwidth(ss(154) + ui->partitionsettings->width()
                             ...
                             + ui->partitionsettings->columnWidth(6));
                if (nwidth > ss(698))
                    windowmove(nwidth < ss(850) ? nwidth : ss(850), ss(465),
                               false);
            } else {
                if (ui->sbpanel->isVisible())
                    ui->functionmenunext->isEnabled()
        ...
        sb::exec(sb::execsrch("reboot") ? "reboot" : "systemctl reboot",
                 nullptr, false, true);
        close();
...
{
    busy();
    QFile::rename(sb::sdir[1] % '/' % cpoint % '_' % pname,
                  sb::sdir[1] % "/H05_" % pname);
    pointupgrade();
    busy(false);
}
...
void systemback::on_pointrename_clicked()
{
    ...
    if (ui->pointpipe1->isChecked() && ui->point1->text() != sb::pnames[0]) {
        if (!ui->point1->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S01_" % sb::pnames[0],
                             sb::sdir[1] % "/S01_" % ui->point1->text()))
            ui->pointpipe1->click();
        else
            dialog = 3;
    }
    ...
    if (ui->pointpipe2->isChecked() && ui->point2->text() != sb::pnames[1]) {
        if (!ui->point2->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S02_" % sb::pnames[1],
                             sb::sdir[1] % "/S02_" % ui->point2->text()))
            ui->pointpipe2->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe3->isChecked() && ui->point3->text() != sb::pnames[2]) {
        if (!ui->point3->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S03_" % sb::pnames[2],
                             sb::sdir[1] % "/S03_" % ui->point3->text()))
            ui->pointpipe3->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe4->isChecked() && ui->point4->text() != sb::pnames[3]) {
        if (!ui->point4->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S04_" % sb::pnames[3],
                             sb::sdir[1] % "/S04_" % ui->point4->text()))
            ui->pointpipe4->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe5->isChecked() && ui->point5->text() != sb::pnames[4]) {
        if (!ui->point5->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S05_" % sb::pnames[4],
                             sb::sdir[1] % "/S05_" % ui->point5->text()))
            ui->pointpipe5->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe6->isChecked() && ui->point6->text() != sb::pnames[5]) {
        if (!ui->point6->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S06_" % sb::pnames[5],
                             sb::sdir[1] % "/S06_" % ui->point6->text()))
            ui->pointpipe6->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe7->isChecked() && ui->point7->text() != sb::pnames[6]) {
        if (!ui->point7->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S07_" % sb::pnames[6],
                             sb::sdir[1] % "/S07_" % ui->point7->text()))
            ui->pointpipe7->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe8->isChecked() && ui->point8->text() != sb::pnames[7]) {
        if (!ui->point8->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S08_" % sb::pnames[7],
                             sb::sdir[1] % "/S08_" % ui->point8->text()))
            ui->pointpipe8->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe9->isChecked() && ui->point9->text() != sb::pnames[8]) {
        if (!ui->point9->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S09_" % sb::pnames[8],
                             sb::sdir[1] % "/S09_" % ui->point9->text()))
            ui->pointpipe9->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe10->isChecked() && ui->point10->text() != sb::pnames[9]) {
        if (!ui->point10->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/S10_" % sb::pnames[9],
                             sb::sdir[1] % "/S10_" % ui->point10->text()))
            ui->pointpipe10->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe11->isChecked() && ui->point11->text() != sb::pnames[10]) {
        if (!ui->point11->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/H01_" % sb::pnames[10],
                             sb::sdir[1] % "/H01_" % ui->point11->text()))
            ui->pointpipe11->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe12->isChecked() && ui->point12->text() != sb::pnames[11]) {
        if (!ui->point12->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/H02_" % sb::pnames[11],
                             sb::sdir[1] % "/H02_" % ui->point12->text()))
            ui->pointpipe12->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe13->isChecked() && ui->point13->text() != sb::pnames[12]) {
        if (!ui->point13->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/H03_" % sb::pnames[12],
                             sb::sdir[1] % "/H03_" % ui->point13->text()))
            ui->pointpipe13->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe14->isChecked() && ui->point14->text() != sb::pnames[13]) {
        if (!ui->point14->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/H04_" % sb::pnames[13],
                             sb::sdir[1] % "/H04_" % ui->point14->text()))
            ui->pointpipe14->click();
        else if (dialog != 3)
            dialog = 3;
    }
    ...
    if (ui->pointpipe15->isChecked() && ui->point15->text() != sb::pnames[14]) {
        if (!ui->point15->text().contains(' ')
            && QFile::rename(sb::sdir[1] % "/H05_" % sb::pnames[14],
                             sb::sdir[1] % "/H05_" % ui->point15->text()))
            ui->pointpipe15->click();
        else if (dialog != 3)
            dialog = 3;
    }
...
{
    if (checked) {
        if (ui->skipfstabrepair->isEnabled())
            ui->skipfstabrepair->setDisabled(true);
        ...
        if (!ui->skipfstabrepair->isEnabled())
            ui->skipfstabrepair->setEnabled(true);
...
{
    busy();
    if (ui->dirchoose->topLevelItemCount() != 0)
        ui->dirchoose->clear();
    QStr pwdrs(sb::fload("/etc/passwd"));
    QSL excl({ "bin", "boot", "cdrom", "dev", "etc", "lib", "lib32", "lib64",
               "opt", "proc", "root", "run", "sbin", "selinux", "srv", "sys",
               "tmp", "usr", "var" });

    for (cQStr &item : QDir("/").entryList(QDir::Dirs | QDir::Hidden
                                           | QDir::NoDotAndDotDot)) {
        QTrWI *twi(new QTrWI);
        twi->setText(0, item);
        ...
        if (excl.contains(item)) {
            ...
            twi->setIcon(0, QIcon(QPixmap(":pictures/dirx.png").scaled(
                                ss(12), ss(12), Qt::IgnoreAspectRatio,
                                Qt::SmoothTransformation)));
            ui->dirchoose->addTopLevelItem(twi);
        } else if (sb::islnxfs('/' % item)) {
            if (ui->function1->text() == tr("Storage directory")
                && sb::isfile('/' % item % "/Systemback/.sbschedule")) {
                twi->setTextColor(0, Qt::green);
                twi->setIcon(0, QIcon(QPixmap(":pictures/isdir.png").scaled(
                                    ss(12), ss(12), Qt::IgnoreAspectRatio,
                                    Qt::SmoothTransformation)));
            }
            ...
            for (cQStr &sitem : QDir('/' % item).entryList(
                     QDir::Dirs | QDir::Hidden | QDir::NoDotAndDotDot)) {
                QTrWI *ctwi(new QTrWI);
                ctwi->setText(0, sitem);
                ...
                if (item == "home" && pwdrs.contains(":/home/" % sitem % ":")) {
                    ...
                    ctwi->setIcon(0, QIcon(QPixmap(":pictures/dirx.png").scaled(
                                         ss(12), ss(12), Qt::IgnoreAspectRatio,
                                         Qt::SmoothTransformation)));
                }
                    ...
                    if (sb::isdir(path % '/' % iname)) {
                        if (sb::islnxfs(path % '/' % iname)) {
                            if (iname == "Systemback") {
                                ...
                                ctwi->setIcon(
                                    0,
                                    QIcon(QPixmap(":pictures/dirx.png").scaled(
                                        ss(12), ss(12), Qt::IgnoreAspectRatio,
                                        Qt::SmoothTransformation)));
                            } else {
                                if (ui->function1->text()
                                    == tr("Storage directory")
                                    && sb::isfile(
                                           path % '/' % iname % '/'
                                           % "/Systemback/.sbschedule")) {
                                    ctwi->setTextColor(0, Qt::green);
                                    ctwi->setIcon(
                                        0,
                                        QIcon(
                                            QPixmap(":pictures/isdir.png")
                                                .scaled(
                                                    ss(12), ss(12),
                                                    Qt::IgnoreAspectRatio,
                                                    Qt::SmoothTransformation)));
                                ...
                                for (cQStr &cdir :
                                     QDir(path % '/' % iname)
                                         .entryList(QDir::Dirs | QDir::Hidden
                                                    | QDir::NoDotAndDotDot)) {
                                    QTrWI *sctwi(new QTrWI);
                            ...
                            ctwi->setTextColor(0, Qt::red);
                            ctwi->setIcon(
                                0, QIcon(QPixmap(":pictures/dirx.png").scaled(
                                       ss(12), ss(12), Qt::IgnoreAspectRatio,
                                       Qt::SmoothTransformation)));
                        }
                    } else
...
void systemback::on_dirchoosecancel_clicked()
{
    ...
    if (ui->function1->text() == tr("Storage directory")) {
        ui->sbpanel->show();
        ui->function1->setText("Systemback");
        ui->functionmenunext->isEnabled() ? ui->functionmenunext->setFocus()
                                          : ui->functionmenuback->setFocus();
    } else {
        ui->livecreatepanel->show();
        ui->function1->setText(tr("Live system create"));
    ...
    if (sb::isdir(ui->dirpath->text())) {
        if (ui->function1->text() == tr("Storage directory")) {
            if (sb::sdir[0] != ui->dirpath->text()) {
                QSL dlst(QDir(sb::sdir[1]).entryList(
                    QDir::AllEntries | QDir::Hidden | QDir::NoDotAndDotDot));

                if (dlst.count() == 0)
                    QDir().rmdir(sb::sdir[1]);
                else if (dlst.count() == 1
                         && sb::isfile(sb::sdir[1] % "/.sbschedule"))
                    sb::remove(sb::sdir[1]);
...
void systemback::on_systemrestore_clicked() { on_fullrestore_clicked(); }
...
{
    if (!arg1.isEmpty()) {
        ui->includeusers->resize(fontMetrics().width(arg1) + ss(30),
                                 ui->includeusers->height());

        if (ui->includeusers->currentIndex() < 2
            || (ui->includeusers->currentIndex() > 1
                && sb::isdir(sb::sdir[1] % '/' % cpoint % '_' % pname % "/home/"
                             % arg1))) {
            if (!ui->restorenext->isEnabled())
                ui->restorenext->setEnabled(true);
...
void systemback::on_livelist_currentItemChanged(QLWI *current)
{
    if (current) {
        if (sb::isfile(
                sb::sdir[2] % '/'
                % sb::left(current->text(), sb::instr(current->text(), " ") - 1)
                % ".sblive")) {
            if (!ui->livedelete->isEnabled())
                ui->livedelete->setEnabled(true);
            ullong isize(sb::fsize(
                sb::sdir[2] % '/'
                % sb::left(current->text(), sb::instr(current->text(), " ") - 1)
                % ".sblive"));

            if (isize > 0 && isize < 4294967295
                && isize * 2 + 104857600 < sb::dfree(sb::sdir[2])
                && !sb::exist(sb::sdir[2] % '/'
                              % sb::left(current->text(),
                                         sb::instr(current->text(), " ") - 1)
                              % ".iso")) {
                if (!ui->liveconvert->isEnabled())
                    ui->liveconvert->setEnabled(true);
            } else if (ui->liveconvert->isEnabled())
                ui->liveconvert->setDisabled(true);

            if (ui->livedevices->currentItem() && isize > 0) {
                if (!ui->livewritestart->isEnabled())
                    ui->livewritestart->setEnabled(true);
            } else if (ui->livewritestart->isEnabled())
                ui->livewritestart->setDisabled(true);

        } else {
            delete current;
            if (ui->livelist->currentItem())
                ui->livelist->currentItem()->setSelected(false);
            if (ui->livedelete->isEnabled())
                ui->livedelete->setDisabled(true);
            if (ui->liveconvert->isEnabled())
                ui->liveconvert->setDisabled(true);
...
void systemback::on_livedevices_currentItemChanged(QTblWI *current,
                                                   QTblWI *previous)
{
    if (current && (!previous || current->row() != previous->row())) {
        ui->livedevices->item(current->row(), 3)->setText("x");
        if (previous)
            ui->livedevices->item(previous->row(), 3)->setText("-");
        if (ui->livelist->currentItem() && !ui->livewritestart->isEnabled())
            ui->livewritestart->setEnabled(true);
    }
}
...
void systemback::on_systemrepair_clicked()
{
    if (ui->grubrepair->isChecked()) {
        ...
        if (ppipe == 1) {
            ...
            if (sb::execsrch("update-grub2")
                && sb::isfile("/var/lib/dpkg/info/grub-" % grub % ".list")) {
                if (ui->grubreinstallrepairdisable->isVisibleTo(
                        ui->repairpanel)) {
                    ui->grubreinstallrepairdisable->hide();
                    ui->grubreinstallrepair->show();
                }
...
void systemback::on_fullrepair_clicked() { on_systemrepair_clicked(); }
void systemback::on_grubrepair_clicked() { on_systemrepair_clicked(); }
...
{
    if (checked && !sb::isfile("/etc/fstab"))
        ui->skipfstabrestore->setChecked(false);
}

void systemback::on_skipfstabrepair_clicked(bool checked)
{
    if (checked && !sb::isfile("/mnt/etc/fstab"))
        ui->skipfstabrepair->setChecked(false);
}
...
void systemback::on_installnext_clicked()
{
    ...
    setMaximumSize(qApp->desktop()->availableGeometry().width() - ss(60),
                   qApp->desktop()->availableGeometry().height() - ss(60));
    if (ui->mountpoint->currentText().startsWith("/home/"))
        ui->mountpoint->setCurrentText(nullptr);

    if (ui->partitionsettings->currentItem()) {
        if (sb::mcheck("/.sbsystemcopy/")
            || sb::mcheck("/.sblivesystemwrite/")) {
            ui->copyback->setDisabled(true);
            on_partitionrefresh_clicked();
            ui->copyback->setEnabled(true);
        } else {
            if (ui->partitionsettings->item(ui->partitionsettings->currentRow(),
                                            3)->text() == "/home"
                && ui->mountpoint->isEnabled()) {
            ...
            for (ushort a(0); a < ui->partitionsettings->rowCount(); ++a)
                if (ui->partitionsettings->item(a, 3)->text() == "/home"
                    && !ui->partitionsettings->item(a, 4)->text().isEmpty()) {
...
void systemback::on_partitionsettings_currentItemChanged(QTblWI *current,
                                                         QTblWI *previous)
{
    if (current && (!previous || current->row() != previous->row())) {
        if (ui->partitionsettingspanel2->isVisible())
            for (ushort a(previous->row() + 1);
                 a < ui->partitionsettings->rowCount()
                 && ui->partitionsettings->item(a, 0)->background() != QBrush();
                 ++a) {
        ...
        uchar type(
            ui->partitionsettings->item(current->row(), 7)->text().toShort()),
            pcount(0);
        ...
        case sb::Extended: {
            ...
            for (ushort a(current->row() + 1);
                 a < ui->partitionsettings->rowCount()
                 && ((type == sb::Extended
                      && ui->partitionsettings->item(a, 0)->text().startsWith(
                             sb::left(
                                 ui->partitionsettings->item(current->row(), 0)
                                     ->text(),
                                 8))
                      && sb::ilike(ui->partitionsettings->item(a, 7)
                                       ->text()
                                       .toUShort(),
                                   { sb::Logical, sb::Emptyspace }))
                     || (type != sb::Extended
                         && ui->partitionsettings->item(a, 0)
                                ->text()
                                .startsWith(
                                    ui->partitionsettings->item(current->row(),
                                                                0)->text())));
                 ++a) {
                ui->partitionsettings->item(a, 0)
                    ->setBackground(QPalette().highlight());
                ...
                if (!mntcheck
                    ...
                    if (((ui->point1->isEnabled()
                          || ui->pointpipe11->isEnabled())
                         && sb::sdir[0].startsWith(
                                ui->partitionsettings->item(a, 3)->text()))
                        || sb::like(ui->partitionsettings->item(a, 3)->text(),
                                    { '_' % tr("Multiple mount points") % '_',
                                      "_/cdrom_", "_/live/image_",
                                      "_/lib/live/mount/medium_" }))
                        mntcheck = true;
            ...
            if (mntd) {
                if (ui->partitiondelete->isEnabled())
                    ui->partitiondelete->setDisabled(true);
            } else if (!ui->partitiondelete->isEnabled())
                ui->partitiondelete->setEnabled(true);

            if (mntd && !mntcheck) {
                if (!ui->umount->isEnabled())
        ...
        case sb::Freespace:
            for (ushort a(0);
                 a < ui->partitionsettings->rowCount() && pcount < 4; ++a)
                if (ui->partitionsettings->item(a, 0)->text().startsWith(
                        sb::left(
                            ui->partitionsettings
                                ->item(ui->partitionsettings->currentRow(), 0)
                                ->text(),
                            8))) {
                    switch (
                        ui->partitionsettings->item(a, 7)->text().toUShort()) {
                    case sb::GPT:
                        pcount = 5;
                    case sb::Primary:
                    case sb::Extended:
        ...
        case sb::Emptyspace:
            ...
            ui->partitionsize->setMaximum(
                (ui->partitionsettings->item(current->row(), 9)
                     ->text()
                     .toULongLong() * 10 / 1048576 + 5) / 10);
        ...
        default:
            ...
            if (ui->partitionsettings->item(current->row(), 3)
                    ->text()
                    .isEmpty()) {
                if (!ui->mountpoint->isEnabled())
                    ui->mountpoint->setEnabled(true);
                ...
                if (ui->mountpoint->currentIndex() == 0) {
                    if (!ui->mountpoint->currentText().isEmpty())
                        ui->mountpoint->setCurrentText(nullptr);
                } else
                    ui->mountpoint->setCurrentIndex(0);
            } else if (ui->partitionsettings->item(current->row(), 3)->text()
                       == "SWAP") {
                if (!ui->mountpoint->isEnabled())
                    ui->mountpoint->setEnabled(true);
                ...
                if (ui->mountpoint->currentText() != "SWAP") {
                    if (ui->mountpoint->currentIndex() > 0)
                        ...
                        ui->mountpoint->setCurrentText(nullptr);
                } else if (ui->partitionsettings->item(current->row(), 4)
                               ->text() == "SWAP") {
                    if (ui->changepartition->isEnabled())
                        ui->changepartition->setDisabled(true);
                    ...
                    ui->changepartition->setEnabled(true);
                       ...
                       == "/home" && ui->userdatafilescopy->isVisible()) {
                if (!ui->mountpoint->isEnabled())
                    ui->mountpoint->setEnabled(true);
                ...
                if (sb::sdir[0].startsWith(
                        ui->partitionsettings->item(current->row(), 3)->text())
                    || sb::like(ui->partitionsettings->item(current->row(), 3)
                                    ->text(),
                                { '_' % tr("Multiple mount points") % '_',
                                  "_/cdrom_", "_/live/image_",
                                  "_/lib/live/mount/medium_" }))
                    ...
                    if (file.open(QIODevice::ReadOnly))
                        while (!file.atEnd()) {
                            QStr cline(
                                file.readLine().trimmed().replace('\t', ' '));

                            if (sb::like(cline.replace("\\040", " "),
                                         { "* " % ui->partitionsettings
                                                      ->item(current->row(), 3)
                                                      ->text() % " *",
                                           "* " % ui->partitionsettings
        ...
        if (pcount == 4) {
            if (ui->newpartition->isEnabled())
                ui->newpartition->setDisabled(true);
...
void systemback::on_changepartition_clicked()
{
    ...
    if (!ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)
             ->text()
             .isEmpty()) {
        if (ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)
                ->text() == "/") {
        ...
        } else if (sb::like(ui->partitionsettings
                                ->item(ui->partitionsettings->currentRow(), 4)
                                ->text(),
                            { "_/home_", "_/boot_", "_/boot/efi_", "_/tmp_",
                              "_/usr_", "_/usr/local_", "_/var_", "_/srv_",
                              "_/opt_" }))
            ui->mountpoint->addItem(
                ui->partitionsettings->item(ui->partitionsettings->currentRow(),
                                            4)->text());
    }
    ...
    if (ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)
        ...
        if (ui->mountpoint->currentText() == "/boot/efi") {
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)
                ->setText("/boot/efi");
            if (ui->partitionsettings->item(ui->partitionsettings->currentRow(),
                                            5)->text() != "vfat")
                ui->partitionsettings->item(ui->partitionsettings->currentRow(),
            ...
            if (sb::isfile("/proc/swaps")) {
                if (QStr('\n' % sb::fload("/proc/swaps")).contains(
                        '\n' % ui->partitionsettings->item(ui->partitionsettings
                                                               ->currentRow(),
                                                           0)->text() % ' ')) {
                    if (ui->partitionsettings
                            ->item(ui->partitionsettings->currentRow(), 6)
                            ->text() == "x")
                        ui->partitionsettings
                            ->item(ui->partitionsettings->currentRow(), 6)
                            ->setText("-");
                } else if (ui->partitionsettings
                               ->item(ui->partitionsettings->currentRow(), 6)
                               ->text() == "-")
                    ui->partitionsettings
                        ->item(ui->partitionsettings->currentRow(), 6)
            ...
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)
                ->setText(ui->mountpoint->currentText());
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)
                ->setText(ui->filesystem->currentText());
    ...
    } else if (ui->partitionsettings->item(ui->partitionsettings->currentRow(),
                                           3)->text() == "/home") {
        ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)
            ->setText("/home");
        ui->userdatafilescopy->setDisabled(true);
    ...
    if (ui->mountpoint->currentIndex() > 0 && ui->mountpoint->currentText()
                                              != "SWAP")
        ui->mountpoint->removeItem(ui->mountpoint->currentIndex());
...
void systemback::on_format_clicked(bool checked)
{
    if (!checked
        && ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)
               ->text() != ui->filesystem->currentText())
        ui->format->setChecked(true);
}
...
void systemback::on_mountpoint_currentTextChanged(const QStr &arg1)
{
    ...
    if (arg1.isEmpty()
        || arg1
           == ui->partitionsettings->item(ui->partitionsettings->currentRow(),
                                          4)->text()
        || (ui->usersettingscopy->isVisible() && arg1.startsWith("/home/"))
        || (arg1 != "/boot/efi"
            && ui->partitionsettings->item(ui->partitionsettings->currentRow(),
                                           9)
                   ->text()
                   .toULongLong() < 268435456)) {
        if (ui->changepartition->isEnabled())
            ui->changepartition->setDisabled(true);
    } else {
        bool check(false);
        ...
        if (check) {
            if (ui->changepartition->isEnabled())
                ui->changepartition->setDisabled(true);
        } else if (sb::like(arg1, { "_/_", "_/home_", "_/boot_", "_/boot/efi_",
                                    "_/tmp_", "_/usr_", "_/usr/local_",
                                    "_/var_", "_/srv_", "_/opt_", "_SWAP_" })) {
            if (!ui->changepartition->isEnabled())
                ui->changepartition->setEnabled(true);
        } else {
            if (ui->changepartition->isEnabled())
            ...
            if (arg1.startsWith('/')
                ...
                if (arg1 == ui->mountpoint->currentText()) {
                    QStr mpname(
                        QStr(arg1 % '_' % sb::rndstr()).replace('/', '_'));
...
void systemback::on_repairpartitionrefresh_clicked()
{
    ...
    for (ushort a(0); a < ui->partitionsettings->rowCount(); ++a)
        if (ui->partitionsettings->item(a, 3)->text().startsWith("/mnt"))
...
{
    if (ui->repairpartition->count() == 0 || !arg1.startsWith("/mnt")
        || sb::like(arg1, { "* *", "*'*", "*\"*", "*//*", "*/_" })
        || (arg1 != "/mnt" && !sb::mcheck("/mnt")) || sb::mcheck(arg1 % '/')) {
        if (ui->repairmount->isEnabled())
            ui->repairmount->setDisabled(true);
    } else {
        if (sb::like(arg1,
                     { "_/mnt_", "_/mnt/home_", "_/mnt/boot_",
                       "_/mnt/boot/efi_", "_/mnt/usr_", "_/mnt/usr/local_",
                       "_/mnt/var_", "_/mnt/opt_" })) {
            if (!ui->repairmount->isEnabled())
...
void systemback::on_repairmount_clicked()
{
    if (!ui->repairmount->text().isEmpty()) {
...

        {
            ...
            for (cQStr &cpath : sb::right(ui->repairmountpoint->currentText(),
                                          -5).split('/')) {
                path.append('/' % cpath);

                if (!sb::isdir("/mnt/" % path)
                    && !QDir().mkdir("/mnt" % path)) {
                    QFile::rename("/mnt" % path,
                                  "/mnt" % path % '_' % sb::rndstr());
                    QDir().mkdir("/mnt" % path);
                }
            }
        }

        if (sb::mount(ui->repairpartition->currentText(),
                      ui->repairmountpoint->currentText())) {
            on_partitionrefresh_clicked();

            if (ui->repairmountpoint->currentIndex() > 0) {
                ui->repairmountpoint->removeItem(
                    ui->repairmountpoint->currentIndex());
                if (ui->repairmountpoint->currentIndex() > 0)
    ...
    if (!ui->repairback->hasFocus() && !ui->repairmountpoint->hasFocus())
        ui->repairback->setFocus();
}
...
void systemback::on_livename_textChanged(const QStr &arg1)
{
    ...
    if (arg1 == "auto") {
        ...
        if (arg1.isEmpty() || arg1.size() > 32
            || sb::like(arg1,
                        { "* *", "*'*", "*\"*", "*_.iso", "*.Iso_", "*.ISo_",
                          "*.IsO_", "*.ISO_", "*.iSo_", "*.iSO_", "*.isO_" })) {
            if (!ui->livenameerror->isVisible())
                                            ...
                                            QIcon(
                                                QPixmap(":pictures/"
                                                    ...
                                                    .scaled(
                                                        ...
                                                        Qt::SmoothTransformation)));
                                    QSL itmlst;
                                    for (ushort b(0); b < ctwi->childCount();
                                         ++b)
                                        itmlst.append(ctwi->child(b)->text(0));
                    ...
                    if (ctwi->icon(0).isNull())
                        ctwi->setIcon(0,
                                      QIcon(QPixmap(":pictures/dir.png").scaled(
                                          ...
                                          Qt::SmoothTransformation)));
                    QSL itmlst;
                    for (ushort b(0); b < ctwi->childCount(); ++b)
                        itmlst.append(ctwi->child(b)->text(0));

                    for (cQStr &siname :
                         QDir("/root" % path % '/' % iname)
                             .entryList(QDir::AllEntries | QDir::Hidden
                                        | QDir::NoDotAndDotDot)) {
                        if (ui->excludedlist->findItems(
                                                  sb::right(path, -1) % '/'
                                                  % iname % '/' % siname,
...
void systemback::on_removeitem_clicked()
{
    ...
    if (file.open(QIODevice::ReadOnly)) {
        while (!file.atEnd()) {
            QStr cline(file.readLine().trimmed());
            if (cline != ui->excludedlist->currentItem()->text())
                excdlst.append(cline % '\n');
        }
        ...
        if (ui->excludedlist->currentItem())
            ui->excludedlist->currentItem()->setSelected(false);
        ui->removeitem->setDisabled(true);
        ui->excludeback->setFocus();
    }
            ...
            if (arg1 == ui->username->text()) {
                ...
                if (rval == 0) {
                    ...
                    if (ui->usernameerror->isVisible())
                        ui->usernameerror->hide();
                    ui->usernamepipe->show();
                } else if (arg1 == ui->username->text()
                           && !ui->usernameerror->isVisible())
                    ui->usernameerror->show();
            }
        }
    }
        ...
        if (!arg1.contains(' ')) {
            ...
            if (arg1 == ui->hostname->text()) {
                ...
                if (file.open(QIODevice::ReadOnly)) {
                    ...
                    } else if (arg1 == ui->hostname->text()
                               && !ui->hostnameerror->isVisible())
...
void systemback::on_rootpassword1_textChanged(const QStr &arg1)
{
    ...
    if (arg1.isEmpty()) {
        ...
        if (ui->rootpassword2->isEnabled())
            ui->rootpassword2->setDisabled(true);
        if (ui->rootpassworderror->isVisible())
            ui->rootpassworderror->hide();
    } else {
        if (!ui->rootpassword2->isEnabled())
...
void systemback::on_schedulerstate_clicked()
{
    if (ui->schedulerstate->isChecked()) {
        ...
        if (sb::schdle[1] != "0")
            ui->daydown->setEnabled(true);
        if (sb::schdle[1] != "7")
            ui->dayup->setEnabled(true);
        if (sb::schdle[2] != "0")
            ui->hourdown->setEnabled(true);
        if (sb::schdle[2] != "23")
            ui->hourup->setEnabled(true);
        if (sb::schdle[3] != "0"
            && (sb::schdle[1] != "0" || sb::schdle[2] != "0"
                || sb::schdle[3].toUShort() > 30))
            ui->minutedown->setEnabled(true);
        if (sb::schdle[3] != "59")
            ui->minuteup->setEnabled(true);
        ui->silentmode->setEnabled(true);

        if (sb::schdle[5] == "off") {
            if (sb::schdle[4] != "10")
                ui->seconddown->setEnabled(true);
            if (sb::schdle[4] != "99")
                ui->secondup->setEnabled(true);
        ...
        if (ui->dayup->isEnabled())
            ui->dayup->setDisabled(true);
        if (ui->daydown->isEnabled())
            ui->daydown->setDisabled(true);
        if (ui->hourup->isEnabled())
            ui->hourup->setDisabled(true);
        if (ui->hourdown->isEnabled())
            ui->hourdown->setDisabled(true);
        if (ui->minuteup->isEnabled())
            ui->minuteup->setDisabled(true);
        if (ui->minutedown->isEnabled())
            ui->minutedown->setDisabled(true);
        if (ui->secondup->isEnabled())
            ui->secondup->setDisabled(true);
        if (ui->seconddown->isEnabled())
            ui->seconddown->setDisabled(true);
        ui->silentmode->setDisabled(true);
        if (ui->windowposition->isEnabled())
...
void systemback::on_dayup_clicked()
{
    ...
    if (sb::schdle[1] == "7")
        ui->dayup->setDisabled(true);
    if (!ui->minutedown->isEnabled() && sb::schdle[3].toUShort() > 0)
        ui->minutedown->setEnabled(true);
}
...
void systemback::on_daydown_clicked()
{
    ...
    if (sb::schdle[1] == "0") {
        if (sb::schdle[2] == "0") {
            ...
            if (ui->minutedown->isEnabled() && sb::schdle[3].toUShort() < 31)
                ui->minutedown->setDisabled(true);
        }
...
{
    sb::schdle[3] = QStr::number(sb::schdle[3].toUShort()
                                 + (sb::schdle[3] == "55" ? 4 : 5));
    if (!cfgupdt)
        cfgupdt = true;
...
void systemback::on_minutedown_clicked()
{
    ...
    if (!ui->minuteup->isEnabled())
        ui->minuteup->setEnabled(true);
    if ((sb::schdle[1] == "0" && sb::schdle[2] == "0" && sb::schdle[3] == "30")
        || sb::schdle[3] == "0")
        ui->minutedown->setDisabled(true);
}
...
{
    sb::schdle[4] = QStr::number(sb::schdle[4].toUShort()
                                 + (sb::schdle[4] == "95" ? 4 : 5));
    if (!cfgupdt)
        cfgupdt = true;
...
void systemback::on_windowposition_currentIndexChanged(const QStr &arg1)
{
    if (ui->schedulepanel->isVisible()) {
        if (arg1 == tr("Top left") && sb::schdle[6] != "topleft") {
            ...
            if (!cfgupdt)
                cfgupdt = true;
        } else if (arg1 == tr("Bottom right") && sb::schdle[6]
                                                 != "bottomright") {
        ...
        if (hfree > 104857600 && sb::dfree("/root") > 104857600
            ...
            while (!file.atEnd()) {
                ...
                if (usr.contains(":/home/")) {
                    ...
                    if (sb::isdir("/home/" % usr) && sb::dfree("/home/" % usr)
                                                     != hfree) {
...
void systemback::interrupt()
{
    if (!intrrpt) {
        if (!intrrptimer->isActive()) {
            ...
            intrrptimer->stop();
            if (ui->pointpipe1->isChecked())
                ui->pointpipe1->setChecked(false);
            if (ui->pointpipe2->isChecked())
                ui->pointpipe2->setChecked(false);
            if (ui->pointpipe3->isChecked())
                ui->pointpipe3->setChecked(false);
            if (ui->pointpipe4->isChecked())
                ui->pointpipe4->setChecked(false);
            if (ui->pointpipe5->isChecked())
                ui->pointpipe5->setChecked(false);
            if (ui->pointpipe6->isChecked())
                ui->pointpipe6->setChecked(false);
            if (ui->pointpipe7->isChecked())
                ui->pointpipe7->setChecked(false);
            if (ui->pointpipe8->isChecked())
                ui->pointpipe8->setChecked(false);
            if (ui->pointpipe9->isChecked())
                ui->pointpipe9->setChecked(false);
            if (ui->pointpipe10->isChecked())
                ui->pointpipe10->setChecked(false);
            if (ui->pointpipe11->isChecked())
                ui->pointpipe11->setChecked(false);
            if (ui->pointpipe12->isChecked())
                ui->pointpipe12->setChecked(false);
            if (ui->pointpipe13->isChecked())
                ui->pointpipe13->setChecked(false);
            if (ui->pointpipe14->isChecked())
                ui->pointpipe14->setChecked(false);
            if (ui->pointpipe15->isChecked())
                ui->pointpipe15->setChecked(false);
            on_pointpipe1_clicked();
            ui->statuspanel->hide();
...
start:
    ...
    for (cQStr &item : QDir(sb::sdir[1]).entryList(QDir::Dirs | QDir::Hidden
                                                   | QDir::NoDotAndDotDot))
        if (sb::like(item, { "_.DELETED_*", "_.S00_*" })) {
            if (prun != tr("Deleting incomplete restore point"))
                prun = tr("Deleting incomplete restore point");
            if (!sb::remove(sb::sdir[1] % '/' % item))
                goto error;
            if (intrrpt)
    ...
    for (uchar a(9); a > 1; --a)
        if (!sb::pnames[a].isEmpty()
            && (a < 9 ? (a > 2 ? sb::pnumber < a + 2 : sb::pnumber == 3)
                      : true))
            ++ppipe;
        ...
        for (uchar a(9); a > 1; --a)
            if (!sb::pnames[a].isEmpty()
                && (a < 9 ? (a > 2 ? sb::pnumber < a + 2 : sb::pnumber == 3)
                          : true)) {
                ++dnum;
                prun = tr("Deleting old restore point") % ' '
                       % QStr::number(dnum) % '/' % QStr::number(ppipe);
                if (!QFile::rename(sb::sdir[1]
                                   % (a < 9 ? QStr("/S0" % QStr::number(a + 1))
                                            : "/S10") % '_' % sb::pnames[a],
                                   sb::sdir[1] % "/.DELETED_" % sb::pnames[a])
                    || !sb::remove(sb::sdir[1] % "/.DELETED_" % sb::pnames[a]))
                    goto error;
                if (intrrpt)
                    goto error;
            }
    ...
    for (uchar a(0);
         a < 9 && sb::isdir(sb::sdir[1] % "/S0" % QStr::number(a + 1) % '_'
                            % sb::pnames[a]);
         ++a)
        if (!QFile::rename(sb::sdir[1] % "/S0" % QStr::number(a + 1) % '_'
                           % sb::pnames[a],
                           sb::sdir[1] % (a < 8 ? "/S0" : "/S")
                           % QStr::number(a + 2) % '_' % sb::pnames[a]))
            goto error;

    if (!QFile::rename(sb::sdir[1] % "/.S00_" % dtime,
                       sb::sdir[1] % "/S01_" % dtime))
        goto error;
    sb::crtfile(sb::sdir[1] % "/.sbschedule");
    if (intrrpt)
        goto error;
...
start:
    ...
    for (schar a(9); a > -1; --a) {
        ...
        ++dnum;
        prun = tr("Deleting restore point") % ' ' % QStr::number(dnum) % '/'
               % QStr::number(ppipe);
        if (!QFile::rename(sb::sdir[1]
                           % (a < 9 ? QStr("/S0" % QStr::number(a + 1))
                                    : "/S10") % '_' % sb::pnames[a],
    ...
    for (uchar a(10); a < 15; ++a) {
        ...
        prun = tr("Deleting restore point") % ' ' % QStr::number(dnum) % '/'
               % QStr::number(ppipe);
        if (!QFile::rename(sb::sdir[1] % "/H0" % QStr::number(a - 9) % '_'
                           % sb::pnames[a],
                           sb::sdir[1] % "/.DELETED_" % sb::pnames[a])
            || !sb::remove(sb::sdir[1] % "/.DELETED_" % sb::pnames[a]))
            goto error;
...
error:
    ...
    if (dialog != 48) {
        if (sb::dfree(sb::sdir[2]) < 104857600
            || (sb::isdir("/home/.sbuserdata") && sb::dfree("/home")
                                                  < 104857600))
            dialog = 29;
        else if (dialog == 0) {
            if (!sb::ThrdDbg.isEmpty())
    ...
    if (sb::isdir("/.sblvtmp"))
        sb::remove("/.sblvtmp");
    if (sb::isdir("/media/.sblvtmp"))
        sb::remove("/media/.sblvtmp");
    if (sb::isdir("/var/.sblvtmp"))
        sb::remove("/var/.sblvtmp");
    if (sb::isdir("/home/.sbuserdata"))
        sb::remove("/home/.sbuserdata");
    if (sb::isdir("/root/.sbuserdata"))
        sb::remove("/root/.sbuserdata");
    if (sb::isdir(sb::sdir[2] % "/.sblivesystemcreate"))
...
start:
    ...
    QStr ckernel(sb::ckname()),
        lvtype(sb::isfile("/usr/share/initramfs-tools/scripts/casper")
                   ? "casper"
                   : "live"),
        ifname;

    if (sb::exist(sb::sdir[2] % "/.sblivesystemcreate")) {
        if (!sb::remove(sb::sdir[2] % "/.sblivesystemcreate"))
            goto error;
        if (intrrpt)
            goto exit;
    }

    if (!QDir().mkdir(sb::sdir[2] % "/.sblivesystemcreate")
        || !QDir().mkdir(sb::sdir[2] % "/.sblivesystemcreate/.disk")
        || !QDir().mkdir(sb::sdir[2] % "/.sblivesystemcreate/" % lvtype)
        || !QDir().mkdir(sb::sdir[2] % "/.sblivesystemcreate/syslinux"))
        goto error;
    ifname = ui->livename->text() == "auto"
                 ? "systemback_live_"
    ...
    while (sb::exist(sb::sdir[2] % '/' % ifname % ".sblive")) {
        ++ncount;
        ncount == 1 ? ifname.append("_1") : ifname
            = sb::left(ifname, sb::rinstr(ifname, "_")) % QStr::number(ncount);
    }

    if (intrrpt)
        goto exit;
    if (!sb::crtfile(sb::sdir[2] % "/.sblivesystemcreate/.disk/info",
                     "Systemback Live (" % ifname % ") - Release "
                     % sb::getarch() % '\n')
        || !sb::copy("/boot/vmlinuz-" % ckernel, sb::sdir[2]
                                                 % "/.sblivesystemcreate/"
                                                 % lvtype % "/vmlinuz"))
        goto error;
    if (intrrpt)
...

    {
        ...
        while (!file.atEnd()) {
            ...
            if (cline.startsWith(guname() % ':')) {
                ...
                for (uchar a(0); a < uslst.count(); ++a)
                    if (a == 4) {
                        fname = sb::left(uslst.at(a),
                                         sb::instr(uslst.at(a), ",") - 1);
                        break;
                    }
    ...
    if (lvtype == "casper") {
        ...
        for (cQStr &item :
             QDir("/usr/share/initramfs-tools/scripts/casper-bottom")
                 .entryList(QDir::Files))
            if (!sb::like(item,
                          { "*integrity_check_", "*mountpoints_", "*fstab_",
                            "*swap_", "*xconfig_", "*networking_",
                            "*disable_update_notifier_",
                            "*disable_hibernation_", "*disable_kde_services_",
                            "*fix_language_selector_", "*disable_trackerd_",
                            "*disable_updateinitramfs_",
                            "*kubuntu_disable_restart_notifications_",
                            "*kubuntu_mobile_session_" })
                && !QFile::setPermissions(
                       ...
                       | QFile::ReadOther))
                goto error;
    } else if (sb::isfile("/usr/share/initramfs-tools/scripts/live-bottom/"
                          "30accessibility")) {
        if (!QFile::setPermissions("/usr/share/initramfs-tools/scripts/"
                                   "live-bottom/30accessibility",
                                   QFile::ReadOwner | QFile::WriteOwner
                                   | QFile::ReadGroup | QFile::ReadOther))
            goto error;
    } else {
        sb::crtfile(
            "/usr/share/initramfs-tools/scripts/init-bottom/sbfstab",
    ...
    if (sb::isfile("/etc/X11/xorg.conf")) {
        sb::crtfile(
            "/usr/share/initramfs-tools/scripts/init-bottom/sbnoxconf",
            "#!/bin/sh\nif [ -s /root/etc/X11/xorg.conf ] && grep noxconf "
            "/proc/cmdline >/dev/null 2>&1\nthen rm "
    ...
    if ((xmntry
         && !sb::remove(
                "/usr/share/initramfs-tools/scripts/init-bottom/sbnoxconf"))
        || !sb::copy("/boot/initrd.img-" % ckernel, sb::sdir[2]
                                                    % "/.sblivesystemcreate/"
                                                    % lvtype % "/initrd.gz"))
        goto error;

    if (sb::isfile("/usr/lib/syslinux/isolinux.bin")) {
        if (!sb::copy("/usr/lib/syslinux/isolinux.bin",
                      sb::sdir[2]
                      % "/.sblivesystemcreate/syslinux/isolinux.bin")
            || !sb::copy("/usr/lib/syslinux/vesamenu.c32",
                         sb::sdir[2]
                         % "/.sblivesystemcreate/syslinux/vesamenu.c32"))
            goto error;
    } else if (!sb::copy("/usr/lib/ISOLINUX/isolinux.bin",
                         sb::sdir[2]
                         % "/.sblivesystemcreate/syslinux/isolinux.bin")
               || !sb::copy("/usr/lib/syslinux/modules/bios/vesamenu.c32",
                            sb::sdir[2]
                            % "/.sblivesystemcreate/syslinux/vesamenu.c32")
               || !sb::copy("/usr/lib/syslinux/modules/bios/libcom32.c32",
                            sb::sdir[2]
                            % "/.sblivesystemcreate/syslinux/libcom32.c32")
               || !sb::copy("/usr/lib/syslinux/modules/bios/libutil.c32",
                            sb::sdir[2]
                            % "/.sblivesystemcreate/syslinux/libutil.c32")
               || !sb::copy("/usr/lib/syslinux/modules/bios/ldlinux.c32",
                            sb::sdir[2]
                            % "/.sblivesystemcreate/syslinux/ldlinux.c32"))
        goto error;

    if (!sb::copy("/usr/share/systemback/splash.png",
                  sb::sdir[2] % "/.sblivesystemcreate/syslinux/splash.png")
        || !sb::lvprpr(ui->userdatainclude->isChecked()))
        goto error;
    QStr ide;

    for (cQStr &item :
         { "/.sblvtmp/cdrom", "/.sblvtmp/dev", "/.sblvtmp/mnt",
           "/.sblvtmp/proc", "/.sblvtmp/run", "/.sblvtmp/srv", "/.sblvtmp/sys",
           "/.sblvtmp/tmp", "/bin", "/boot", "/etc", "/lib", "/lib32", "/lib64",
           "/opt", "/sbin", "/selinux", "/usr", "/initrd.img",
           "/initrd.img.old", "/vmlinuz", "/vmlinuz.old" })
        if (sb::exist(item))
            ide.append(' ' % item);
    ...
    if (sb::isdir(sb::sdir[2] % "/.sblivesystemcreate/userdata")) {
        ...
        if (sb::isdir(sb::sdir[2] % "/.sblivesystemcreate/userdata/root"))
            ide.append(' ' % sb::sdir[2]
                       % "/.sblivesystemcreate/userdata/root");
    } else {
        if (sb::isdir("/home/.sbuserdata"))
    ...
    if (sb::exec(
            "mksquashfs" % ide % ' ' % sb::sdir[2]
            % "/.sblivesystemcreate/.systemback /media/.sblvtmp/media "
              "/var/.sblvtmp/var " % sb::sdir[2] % "/.sblivesystemcreate/"
            % lvtype
            % "/filesystem.squashfs -info -b 1M -no-duplicates -no-recovery "
              "-always-use-fragments -e /etc/fstab -e /etc/mtab -e "
              "/etc/udev/rules.d/70-persistent-cd.rules -e "
              "/etc/udev/rules.d/70-persistent-net.rules") > 0) {
        dialog = 26;
        goto error;
    }
    ...
    if (QFile(sb::sdir[2] % "/.sblivesystemcreate/" % lvtype
              % "/filesystem.squashfs").size() > 4294967295)
        rpart = "root=LABEL=SBROOT ";

    if (xmntry) {
        grxorg = "menuentry \"" % tr("Boot Live without xorg.conf file")
                 % "\" {\n  set gfxpayload=keep\n  linux /" % lvtype
                 % "/vmlinuz " % rpart % "boot=" % lvtype
                 % " noxconf quiet splash\n  initrd /" % lvtype
                 % "/initrd.gz\n}\n\n";
        srxorg = "label noxconf\n  menu label "
                 % tr("Boot Live without xorg.conf file") % "\n  kernel /"
                 % lvtype % "/vmlinuz\n  append " % rpart % "boot=" % lvtype
                 % " initrd=/" % lvtype % "/initrd.gz noxconf quiet splash\n\n";
    }

    if (sb::getarch() == "amd64"
        && sb::isfile("/usr/share/systemback/efi-amd64.bootfiles")
        && (sb::exec(
                "tar -xJf /usr/share/systemback/efi-amd64.bootfiles -C "
                % sb::sdir[2]
                % "/.sblivesystemcreate --no-same-owner --no-same-permissions")
            > 0 || !sb::copy("/usr/share/systemback/splash.png",
                             sb::sdir[2]
                             % "/.sblivesystemcreate/boot/grub/splash.png")
            || !sb::crtfile(
                   sb::sdir[2] % "/.sblivesystemcreate/boot/grub/grub.cfg",
                   "if loadfont /boot/grub/font.pf2\nthen\n  set "
                   "gfxmode=auto\n  insmod efi_gop\n  insmod efi_uga\n  "
                   "insmod gfxterm\n  terminal_output gfxterm\nfi\n\nset "
                   "theme=/boot/grub/theme.cfg\n\nmenuentry \""
                   % tr("Boot Live system")
                   % "\" {\n  set gfxpayload=keep\n  linux /" % lvtype
                   % "/vmlinuz " % rpart % "boot=" % lvtype
                   % " quiet splash\n  initrd /" % lvtype
                   % "/initrd.gz\n}\n\nmenuentry \""
                   % tr("Boot Live in safe graphics mode")
                   % "\" {\n  set gfxpayload=keep\n  linux /" % lvtype
                   % "/vmlinuz " % rpart % "boot=" % lvtype
                   % " xforcevesa nomodeset quiet splash\n  initrd /" % lvtype
                   % "/initrd.gz\n}\n\n" % grxorg % "menuentry \""
                   % tr("Boot Live in debug mode")
                   % "\" {\n  set gfxpayload=keep\n  linux /" % lvtype
                   % "/vmlinuz " % rpart % "boot=" % lvtype % "\n  initrd /"
                   % lvtype % "/initrd.gz\n}\n")
            || !sb::crtfile(
                   sb::sdir[2] % "/.sblivesystemcreate/boot/grub/theme.cfg",
                   "title-color: \"white\"\ntitle-text: \"Systemback Live ("
                   % ifname
                   % ")\"\ntitle-font: \"Sans Regular 16\"\ndesktop-color: "
                     ...
                     "item_spacing = 5\n}\n\n+ vbox {\n  top = 100%\n  left = "
                     "2%\n  "
                     "+ label {text = \"" % tr("Press 'E' key to edit")
                   % "\" font = \"Sans 10\" color = "
                     "\"white\" align = "
                     "\"left\"}\n}\n")
            || !sb::crtfile(sb::sdir[2]
                            % "/.sblivesystemcreate/boot/grub/loopback.cfg",
                            ...
                            % " xforcevesa nomodeset quiet splash\n  initrd /"
                            % lvtype % "/initrd.gz\n}\n\n" % grxorg
                            % "menuentry \"" % tr("Boot Live in debug mode")
                            % "\" {\n  set gfxpayload=keep\n  linux /" % lvtype
                            % "/vmlinuz " % rpart % "boot=" % lvtype
    ...
    if (!sb::crtfile(
            sb::sdir[2] % "/.sblivesystemcreate/syslinux/syslinux.cfg",
            "default vesamenu.c32\nprompt 0\ntimeout 100\n\nmenu title "
            "Systemback Live (" % ifname % ")\nmenu tabmsg "
            % tr("Press TAB key to edit")
            % "\nmenu background splash.png\n\nlabel live\n  menu label "
            % tr("Boot Live system") % "\n  kernel /" % lvtype
            % "/vmlinuz\n  append " % rpart % "boot=" % lvtype % " initrd=/"
            % lvtype % "/initrd.gz quiet splash\n\nlabel safe\n  menu label "
            % tr("Boot Live in safe graphics mode") % "\n  kernel /" % lvtype
            % "/vmlinuz\n  append " % rpart % "boot=" % lvtype % " initrd=/"
            % lvtype % "/initrd.gz xforcevesa nomodeset quiet splash\n\n"
            % srxorg % "label debug\n  menu label "
            % tr("Boot Live in debug mode") % "\n  kernel /" % lvtype
            % "/vmlinuz\n  append " % rpart % "boot=" % lvtype % " initrd=/"
            % lvtype % "/initrd.gz\n"))
    ...
    if (intrrpt)
        goto exit;
    if (!sb::remove(sb::sdir[2] % "/.sblivesystemcreate/.systemback"))
        goto error;
    if (intrrpt)
        goto exit;
    if (sb::isdir(sb::sdir[2] % "/.sblivesystemcreate/userdata")
    ...
    if (sb::exec("tar -cf " % sb::sdir[2] % '/' % ifname % ".sblive -C "
                 % sb::sdir[2] % "/.sblivesystemcreate .") > 0) {
        if (sb::exist(sb::sdir[2] % '/' % ifname % ".sblive"))
            sb::remove(sb::sdir[2] % '/' % ifname % ".sblive");
        dialog = 27;
        goto error;
    }

    if (!QFile::setPermissions(sb::sdir[2] % '/' % ifname % ".sblive",
                               QFile::ReadOwner | QFile::WriteOwner
...
error:
    ...
    return;
exit:
    if (sb::isdir(sb::sdir[2] % "/.sblivesystemconvert"))
        sb::remove(sb::sdir[2] % "/.sblivesystemconvert");
    if (sb::isfile(sb::sdir[2] % '/'
                   % sb::left(ui->livelist->currentItem()->text(),
                              sb::instr(ui->livelist->currentItem()->text(),
                                        " ") - 1) % ".iso"))
        sb::remove(sb::sdir[2] % '/'
...
start:
    ...
    if ((sb::exist(sb::sdir[2] % "/.sblivesystemconvert")
         && !sb::remove(sb::sdir[2] % "/.sblivesystemconvert"))
        || !QDir().mkdir(sb::sdir[2] % "/.sblivesystemconvert"))
        goto error;
    sb::ThrdLng[0] = sb::fsize(
        sb::sdir[2] % '/'
        % sb::left(ui->livelist->currentItem()->text(),
                   sb::instr(ui->livelist->currentItem()->text(), " ") - 1)
        % ".sblive");
    ...
    if (sb::exec(
            ...
            % sb::left(ui->livelist->currentItem()->text(),
                       sb::instr(ui->livelist->currentItem()->text(), " ") - 1)
            % ".sblive -C " % sb::sdir[2]
            % "/.sblivesystemconvert --no-same-owner --no-same-permissions")
        > 0) {
        dialog = 47;
        goto error;
    }
    ...
    if (!QFile::rename(
            sb::sdir[2] % "/.sblivesystemconvert/syslinux/syslinux.cfg",
            sb::sdir[2] % "/.sblivesystemconvert/syslinux/isolinux.cfg")
        || !QFile::rename(sb::sdir[2] % "/.sblivesystemconvert/syslinux",
                          sb::sdir[2] % "/.sblivesystemconvert/isolinux"))
        goto error;
    if (intrrpt)
        goto exit;
    prun = tr("Converting Live system image") % '\n' % tr("process") % " 2/2";
    ...
    if (sb::exec(
            "genisoimage -r -V sblive -cache-inodes -J -l -b "
            "isolinux/isolinux.bin -no-emul-boot -boot-load-size 4 "
            "-boot-info-table -c isolinux/boot.cat -o " % sb::sdir[2] % '/'
            % sb::left(ui->livelist->currentItem()->text(),
                       sb::instr(ui->livelist->currentItem()->text(), " ") - 1)
            % ".iso " % sb::sdir[2] % "/.sblivesystemconvert") > 0) {
        if (sb::isfile(sb::sdir[2] % '/'
                       % sb::left(ui->livelist->currentItem()->text(),
                                  sb::instr(ui->livelist->currentItem()->text(),
                                            " ") - 1) % ".iso"))
    ...
    if (intrrpt)
        goto exit;
    if (sb::exec("isohybrid " % sb::sdir[2] % '/'
                 % sb::left(ui->livelist->currentItem()->text(),
                            sb::instr(ui->livelist->currentItem()->text(), " ")
                            - 1) % ".iso") > 0
        || !QFile::setPermissions(
    ...
    sb::fssync();
    sb::crtfile("/proc/sys/vm/drop_caches", "3");
    ui->livelist->currentItem()->setText(
        sb::left(ui->livelist->currentItem()->text(),
                 sb::rinstr(ui->livelist->currentItem()->text(), " "))
        % "sblive+iso)");
    ui->liveconvert->setDisabled(true);
    ui->statuspanel->hide();
    ui->mainpanel->show();
...
void systemback::on_partitiondelete_clicked()
{
    ...
    switch (
        ui->partitionsettings->item(ui->partitionsettings->currentItem()->row(),
                                    7)
            ->text()
            .toShort()) {
    case sb::Extended:
...
void systemback::on_newpartition_clicked()
{
    ...
    QStr dev(sb::left(
        ...
        8));
    ullong start(
        ui->partitionsettings->item(ui->partitionsettings->currentRow(), 8)
            ->text()
            .toULongLong());
    uchar type;

    switch (ui->partitionsettings->item(ui->partitionsettings->currentRow(), 7)
                ->text()
                .toUShort()) {
    case sb::Freespace: {
        ...
        if (pcount > 2 && (fcount > 1 || ui->partitionsize->value()
                                         < ui->partitionsize->maximum())) {
            if (!sb::mkpart(dev, start,
                            ui->partitionsettings
                                ->item(ui->partitionsettings->currentRow(), 9)
                                ->text()
                                .toULongLong(),
                            sb::Extended))
                goto end;
...
exec:
    sb::mkpart(
        dev, start,
        ui->partitionsize->value() == ui->partitionsize->maximum()
            ? ui->partitionsettings->item(ui->partitionsettings->currentRow(),
                                          9)
                  ->text()
                  .toULongLong()
            : ullong(ui->partitionsize->value()) * 1048576,
        type);
end:
    on_partitionrefresh2_clicked();
    busy(false);
