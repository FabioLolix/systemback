...
systemback::systemback(QWidget *parent) : QMainWindow(parent, Qt::FramelessWindowHint), ui(new Ui::systemback)
{
    ...
    else
    {
        ...
        if((qApp->arguments().count() == 2 || (qApp->arguments().count() == 3 && qApp->arguments().value(2).startsWith("/dev/"))) && qApp->arguments().value(1).startsWith("dialog") && qApp->arguments().value(1).length() > 7)
        {
            dialog = sb::right(qApp->arguments().value(1), -7).toShort();
            if(sb::ilike(dialog, QSIL() << 31 << 36 << 51 << 52)) dialogdev = qApp->arguments().value(2);
        }
        else if(geteuid() != 0)
            dialog = 17;
    ...
    else
    {
        ...
        ui->partitionsettings->setHorizontalHeaderItem(2, cmptext);
        ui->partitionsettings->setHorizontalHeaderItem(3, nmptext);
        ui->partitionsettings->setHorizontalHeaderItem(4, fstext);
        ui->partitionsettings->setHorizontalHeaderItem(5, frmttext);
            ...
            while(! in.atEnd())
            {
                ...
                if(sb::like(usrs, QSL() << "_sudo:*" << "_admins:*"))
                {
                    ...
                    for(uchar a(0) ; a < lst.count() ; ++a)
                        if(ui->admins->findText(lst.at(a)) == -1) ui->admins->addItem(lst.at(a));
                }
            }
    ...
    if(! utblock)
    {
        ...
        if(utimer->interval() == 100)
        {
            ...
            if(! sstart)
            {
                ...
                ui->usersettingscopy->resize(fontMetrics().width(ui->usersettingscopy->text()) + 28, 20);
            ...
            if(sb::exec("modprobe -n overlayfs", NULL, true) == 0)
                ickernel = true;
            else if(sb::exec("modprobe -n aufs", NULL, true) == 0)
                ickernel = true;
            else if(sb::exec("modprobe -n unionfs", NULL, true) == 0)
                ickernel = true;
            else if(isfile("/usr/bin/unionfs-fuse"))
                ickernel = true;
            else
                ickernel = false;
        ...
        else if(ui->statuspanel->isVisible())
        {
            ...
            else if(prun.startsWith(tr("Creating Live system")))
            {
                ...
                if(cperc != -1)
                {
                    ...
                    if(cperc < 101)
                        if(ui->progressbar->value() < cperc) ui->progressbar->setValue(cperc);
                }
                else if(ui->progressbar->maximum() == 100)
                    ui->progressbar->setMaximum(0);
            }
            else if(prun == tr("Writing Live image to USB device"))
            {
                if(ui->interruptdisable->isVisible())
                {
                    ui->interruptdisable->hide();
            ...
            {
                if(sb::like(prun, QSL() << '_' % tr("Deleting restore point") % "*" << '_' % tr("Deleting old restore point") % "*" << '_' % tr("Deleting incomplete restore point") % '_'))
                {
                    if(ui->interruptdisable->isVisible())
                    {
                        ui->interruptdisable->hide();
...
{
    if(ui->admins->currentText().isNull())
    {
        ...
        while(! in.atEnd())
        {
            ...
            if(sb::like(usr, QSL() << "*:x:1000:10*" << "*:x:1001:10*" << "*:x:1002:10*" << "*:x:1003:10*" << "*:x:1004:10*" << "*:x:1005:10*" << "*:x:1006:10*" << "*:x:1007:10*" << "*:x:1008:10*" << "*:x:1009:10*" << "*:x:1010:10*" << "*:x:1011:10*" << "*:x:1012:10*" << "*:x:1013:10*" << "*:x:1014:10*" << "*:x:1015:10*"))
            {
                ui->admins->addItem(sb::left(usr, sb::instr(usr, ":") -1));
                break;
            }
        }
...
{
    if(ui->copypanel->isVisible() || ui->excludepanel->isVisible() || ui->choosepanel->isVisible()) isMaximized() ? showNormal() : showMaximized();
}
...
{
    if(ui->buttonspanel->isVisible() && ui->windowmaximize->foregroundRole() == QPalette::Highlight) (ui->windowmaximize->text() == "□") ? showMaximized() : showNormal();
}
...
{
    if(! isMaximized()) qApp->setOverrideCursor(Qt::SizeFDiagCursor);
}
...
void systemback::chssmove()
{
    if(! isMaximized())
    {
        short nwidth(QCursor::pos().x() - x() + width() - ui->chooseresize->x() - ui->chooseresize->MouseX - 1), nheight(QCursor::pos().y() - y() + height() - ui->chooseresize->y() - ui->chooseresize->MouseY - 24);
...
void systemback::xcldmove()
{
    if(! isMaximized())
    {
        short nwidth(QCursor::pos().x() - x() + width() - ui->excluderesize->x() - ui->excluderesize->MouseX - 1), nheight(QCursor::pos().y() - y() + height() - ui->excluderesize->y() - ui->excluderesize->MouseY - 24);
...
void systemback::restore()
{
    ...
    if(mthd < 3)
    {
        if(isfile("/etc/fstab") && isfile(sb::sdir[1] % '/' % cpoint % '_' % pname % "/etc/fstab"))
        {
            if(sb::fload("/etc/fstab") == sb::fload(sb::sdir[1] % '/' % cpoint % '_' % pname % "/etc/fstab"))
            {
                fcmp = true;
        ...
        if(! ui->autorestoreoptions->isChecked())
            if(ui->skipfstabrestore->isChecked()) sfstab = true;
    }
...
void systemback::repair()
{
    ...
    else
    {
        ...
        if(mthd < 3)
        {
            if(isfile("/mnt/etc/fstab") && isfile(sb::sdir[1] % '/' % cpoint % '_' % pname % "/etc/fstab"))
            {
                if(sb::fload("/mnt/etc/fstab") == sb::fload(sb::sdir[1] % '/' % cpoint % '_' % pname % "/etc/fstab"))
                {
                    fcmp = true;
            ...
            if(! ui->autorepairoptions->isChecked())
                if(ui->skipfstabrepair->isChecked()) sfstab = true;
        }
        ...
        if(pname == tr("Live image"))
        {
            ...
            if(isfile("/cdrom/casper/filesystem.squashfs"))
                mdev = "/cdrom/casper/filesystem.squashfs";
            else
                mdev = isfile("/lib/live/mount/medium/live/filesystem.squashfs") ? "/lib/live/mount/medium/live/filesystem.squashfs" : "/live/image/live/filesystem.squashfs";
...
start:;
    ...
    for(uchar a(0) ; a < ui->partitionsettings->verticalHeader()->count() ; ++a)
    {
        ...
        {
            if(ui->partitionsettings->item(a, 3)->text() != "/home" || ui->partitionsettings->item(a, 2)->text().isEmpty())
            {
                if(ui->partitionsettings->item(a, 5)->text() == "x")
                    msort.append(ui->partitionsettings->item(a, 3)->text() % ' ' % ui->partitionsettings->item(a, 4)->text() % ' ' % ui->partitionsettings->item(a, 0)->text());
                else
                    msort.append(ui->partitionsettings->item(a, 3)->text() % " - " % ui->partitionsettings->item(a, 0)->text());
            }
        }
    }
    ...
    for(uchar a(0) ; a < msort.count() ; ++a)
    {
        ...
        if(mset[1] != "-")
        {
            ...
            {
                sb::exec("parted -s " % sb::left(mset[2], 8) % " set " % sb::right(mset[2], -8) % " boot on");
                rv = sb::exec("mkfs.vfat -F 32 " % mset[2]);
            }
...
start:;
    ...
    if(prun.isEmpty()) return;
    sb::SBThrd.ThrdLng = sb::fsize(sb::sdir[2] % '/' % sb::left(ui->livelist->currentItem()->text(), sb::instr(ui->livelist->currentItem()->text(), " ") - 1) % ".sblive");
    QStr lrdir;

    if(sb::SBThrd.ThrdLng < 4294967295)
    {
        lrdir = "sblive";
        bool stract(0);
    ...
    if(sb::exec("parted -s " % ldev % " set 1 boot On") > 0) goto error;
    if(sb::exec("parted -s " % ldev % " set 1 lba On") > 0) goto error;
    if(prun.isEmpty()) return;

    if(sb::exist("/.sblivesystemwrite"))
    {
        if(sb::mcheck("/.sblivesystemwrite/sblive"))
            if(! sb::umount("/.sblivesystemwrite/sblive")) goto error;

        if(sb::mcheck("/.sblivesystemwrite/sbroot"))
            if(! sb::umount("/.sblivesystemwrite/sbroot")) goto error;
    ...
    if(sb::dfree("/.sblivesystemwrite/" % lrdir) < sb::SBThrd.ThrdLng + 52428800)
    {
        dialog = 42;
        goto error;
    }
...
void systemback::dialogopen()
{
    ...
    else if(sb::ilike(dialog, QSIL() << 31 << 36))
        sb::exec("systemback dialog=" % QStr::number(dialog) % ' ' % dialogdev, NULL, false, true);
    else
        sb::exec("systemback dialog=" % QStr::number(dialog), NULL, false, true);
}
...
void systemback::windowmove(ushort nwidth, ushort nheight)
{
    ...
    if(width() != nwidth || height() != nheight)
    {
        ...
        if(wmovex < 0) wmovex = 0;
        wmovey = y() + (height() - nheight) / 2;
        if(wmovey < 0) wmovey = 0;
        wgeom[0] = wmovex;
        wgeom[1] = wmovey;
        wgeom[2] = nwidth;
        wgeom[3] = nheight;
...
void systemback::keyPressEvent(QKeyEvent *ev)
{
    ...
    case Qt::Key_Return:
        ...
        if(ui->sbpanel->isVisible())
        {
            ...
            else if(ui->point15->hasFocus())
            {
                if(ui->pointpipe15->isChecked()) on_pointrename_clicked();
            }
        }
    ...
    case Qt::Key_F5:
        ...
        if(ui->sbpanel->isVisible())
        {
            ...
            else if(ui->point14->hasFocus())
            {
                ...
                if(ui->point15->text() != sb::pnames[14]) ui->point15->setText(sb::pnames[14]);
        }
...
void systemback::on_admins_currentIndexChanged(const QStr &arg1)
{
    ...
    if(! hash.isEmpty())
    {
        if(QStr(crypt("", hash.toStdString().c_str())) == hash)
        {
...
void systemback::on_copymenu_clicked()
{
    ...
    if(ui->partitionsettings->currentItem())
    {
        if(sb::mcheck("/.sbsystemcopy/") || sb::mcheck("/.sblivesystemwrite/"))
            on_partitionupdate_clicked();
        else if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text() == "/home" && ! ui->mountpoint->isEnabled())
...
void systemback::on_partitionupdate_clicked()
{
    ...
    for(ushort a(0) ; a < dlst.count() ; ++a)
    {
        sb::SBThrd.ThrdStr[0] = "/dev/" % dlst.at(a);

        if(sb::like(sb::SBThrd.ThrdStr[0], QSL() << "_/dev/sd*" << "_/dev/hd*") && sb::SBThrd.ThrdStr[0].length() > 8)
        {
            ...
            if(sb::SBThrd.ThrdLng >= 1048576)
            {
                ...
                else
                {
                    ...
                    {
                        if(QStr('\n' % mntlst).count("\n/dev/disk/by-uuid/" % QStr(sb::SBThrd.FSUUID) % ' ') > 1)
                            mpt->setText(tr("Multiple mount points"));
                        else
                        {
                            QStr mnt(sb::right(mntlst, - sb::instr(mntlst, "/dev/disk/by-uuid/" % QStr(sb::SBThrd.FSUUID) % ' ')));
                            short spc(sb::instr(mnt, " "));
                            mpt->setText(sb::replace(sb::mid(mnt, spc + 1, sb::instr(mnt, " ", spc + 1) - spc - 1) , "\\040", " "));
                        }
                    }
                ...
                ui->partitionsettings->setItem(sn, 2, mpt);
                if(ui->partitionsettings->item(sn, 2)->text().isEmpty()) ui->repairpartition->addItem(ui->partitionsettings->item(sn, 0)->text());
                QTblWI *fs(new QTblWI(fsystem));
                fs->setTextAlignment(Qt::AlignCenter);
                ui->partitionsettings->setItem(sn, 4, fs);
                QTblWI *frmt(new QTblWI("-"));
                frmt->setTextAlignment(Qt::AlignCenter);
                ui->partitionsettings->setItem(sn, 5, frmt);
            }
        }
    }
...
void systemback::on_umount_clicked()
{
    ...
    if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text() == "SWAP")
    {
...
void systemback::on_livedevicesupdate_clicked()
{
    ...
    {
        sb::SBThrd.ThrdStr[0] = sb::left(devs.at(a), sb::instr(devs.at(a), "_") - 1);
        ++sn;
        ui->livedevices->setRowCount(sn + 1);
        QTblWI *dev(new QTblWI(sb::SBThrd.ThrdStr[0]));
        dev->setTextAlignment(Qt::AlignCenter);
        ui->livedevices->setItem(sn, 0, dev);
        sb::SBThrd.ThrdType = sb::Dsize;
        sb::SBThrd.start();
        sb::SBThrd.thrdelay();
        QTblWI *size(new QTblWI);
        ...
        if(usr.contains(":/home/"))
        {
            ...
            if(isdir("/home/" % usr))
            {
                ...
                for(ushort a(0) ; a < dlst.count() ; ++a)
                {
                    ...
                    if((iname.startsWith(".") && ui->pointexclude->isChecked()) || (! iname.startsWith(".") && ui->liveexclude->isChecked()))
                    {
                        if(ui->excludedlist->findItems(iname, Qt::MatchExactly).isEmpty() && ! sb::like(iname, QSL() << "_.gvfs_" << "_.Xauthority_"))
                        {
...
void systemback::on_pointrename_clicked()
{
    ...
    if(ui->pointpipe1->isChecked() && ui->point1->text() != sb::pnames[0])
    {
        if(ui->point1->text().contains(" "))
            dialog = 3;
        else if(QFile::rename(sb::sdir[1] % "/S01_" % sb::pnames[0], sb::sdir[1] % "/S01_" % ui->point1->text()))
            ui->pointpipe1->click();
        else
            dialog = 3;
    }
...
void systemback::on_installnext_clicked()
{
    ...
    if(ui->partitionsettings->currentItem())
    {
        ...
        {
            if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text() == "/home" && ui->mountpoint->isEnabled())
            {
            ...
            {
                if(ui->partitionsettings->item(a, 2)->text() == "/home" && ! ui->partitionsettings->item(a, 3)->text().isEmpty())
                {
...
void systemback::on_partitionsettings_currentItemChanged(QTblWI *current, QTblWI *previous)
{
    ...
    if(current)
    {
        if(current->row() != prow)
        {
            if(ui->partitionsettings->item(current->row(), 2)->text().isEmpty())
            {
            ...
            else if(ui->partitionsettings->item(current->row(), 2)->text() == "SWAP")
            {
                if(! ui->mountpoint->isEnabled()) ui->mountpoint->setEnabled(true);
            ...
            else if(ui->partitionsettings->item(current->row(), 2)->text() == "/home" && ui->userdatafilescopy->isVisible())
            {
                if(! ui->mountpoint->isEnabled()) ui->mountpoint->setEnabled(true);
            ...
            else
            {
                ...
                if(ui->partitionsettings->item(current->row(), 2)->text() == tr("Multiple mount points") || ui->partitionsettings->item(current->row(), 2)->text() == "/cdrom" || ui->partitionsettings->item(current->row(), 3)->text() == "/live/image" || ui->partitionsettings->item(current->row(), 3)->text() == "/lib/live/mount/medium")
                    mntcheck = true;
                else if(isfile("/etc/fstab"))
                {
                    ...
                    while(! in.atEnd())
                    {
                        ...
                        if(sb::like(sb::replace(cline, "\\040", " "), QSL() << "* " % ui->partitionsettings->item(current->row(), 2)->text() % " *" << "* " % ui->partitionsettings->item(current->row(), 2)->text() % "/ *"))
                        {
                            mntcheck = true;
                            break;
                        }
...
void systemback::on_changepartition_clicked()
{
    ...
    {
        if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/")
        {
            ui->copynext->setDisabled(true);
            ui->mountpoint->addItem("/");
        }
        else if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/home" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/boot" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/boot/efi" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/tmp" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/usr" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/usr/local" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/var" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/srv" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text() == "/opt")
            ui->mountpoint->addItem(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text());
    }

    if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text().isEmpty())
    {
        if(ui->mountpoint->currentText() == "/boot/efi")
        {
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->setText("/boot/efi");
            if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)->text() != "vfat") ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)->setText("vfat");
            if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->text() == "-") ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->setText("x");
        }
        else if(ui->mountpoint->currentText() == "SWAP")
        {
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->setText("SWAP");
            if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)->text() != "swap") ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)->setText("swap");

            if(isfile("/proc/swaps"))
            {
                if(QStr('\n' % sb::fload("/proc/swaps")).contains('\n' % ui->partitionsettings->item(ui->partitionsettings->currentRow(), 0)->text() % ' '))
                {
                    if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->text() == "x") ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->setText("-");
                }
                else if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->text() == "-")
                    ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->setText("x");
            }
            else if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->text() == "-")
                ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->setText("x");
        }
        else
        {
            ...
            ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)->setText(ui->filesystem->currentText());

            if(ui->format->isChecked())
            {
                if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->text() == "-") ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->setText("x");
            }
            else if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->text() == "x")
                ui->partitionsettings->item(ui->partitionsettings->currentRow(), 5)->setText("-");
...
{
    if(! ui->format->isChecked() && ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)->text() != arg1) ui->format->setChecked(true);
}

void systemback::on_format_clicked(bool checked)
{
    if(! checked && ui->partitionsettings->item(ui->partitionsettings->currentRow(), 4)->text() != ui->filesystem->currentText()) ui->format->setChecked(true);
}

void systemback::on_mountpoint_currentTextChanged(const QStr &arg1)
{
    if(arg1 == "/boot/efi" || arg1 == "SWAP" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text() == "/home" || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text() == "SWAP")
    {
        if(ui->filesystem->isEnabled())
        {
            ui->filesystem->setDisabled(true);
    ...
    if(arg1.isEmpty() || arg1 == ui->partitionsettings->item(ui->partitionsettings->currentRow(), 3)->text())
    {
        if(ui->changepartition->isEnabled()) ui->changepartition->setDisabled(true);
    }
    else
    {
        ...
        if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text() == "/home" && arg1 != "/home")
            check = true;
        else if(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 2)->text() == "SWAP" && arg1 != "SWAP")
            check = true;
        else if(arg1 != "SWAP")
        {
            for(uchar a(0) ; a < ui->partitionsettings->verticalHeader()->count() ; ++a)
            {
                if(ui->partitionsettings->item(a, 3)->text() == arg1)
                {
                    check = true;
                    break;
                }
        ...
        else
        {
            ...
            if(arg1.startsWith("/") && arg1.count("/") == 1 && ! sb::like(arg1, QSL() << "* *" << "*'*" << "*\"*"))
            {
                sb::delay(300);

                if(arg1 == ui->mountpoint->currentText())
                {
                    QStr mpname("/tmp/" % arg1 % "_" % sb::rndstr());

                    if(QDir().mkdir(mpname))
                    {
                        sb::remove(mpname);
                        ui->changepartition->setEnabled(true);
                    }
...
void systemback::on_repairpartitionupdate_clicked()
{
    ...
    for(uchar a(0) ; a < ui->partitionsettings->verticalHeader()->count() ; ++a)
    {
        if(ui->partitionsettings->item(a, 2)->text().startsWith("/mnt")) sb::umount(ui->partitionsettings->item(a, 0)->text());
    }
...
{
    if(arg1.isEmpty() || ui->repairpartition->count() == 0)
    {
        if(ui->repairmount->isEnabled()) ui->repairmount->setDisabled(true);
    }
    else if(! arg1.startsWith("/mnt") || sb::like(arg1, QSL() << "* *" << "*'*" << "*\"*" << "*//*" << "*/_"))
    {
        if(ui->repairmount->isEnabled()) ui->repairmount->setDisabled(true);
    }
    else if(! sb::mcheck("/mnt") && arg1 != "/mnt")
    {
        if(ui->repairmount->isEnabled()) ui->repairmount->setDisabled(true);
    }
    else if(! sb::mcheck(arg1 % '/'))
    {
...
void systemback::on_repairmount_clicked()
{
    if(! ui->repairmount->text().isEmpty())
    {
        ...
        if(sb::mount(ui->repairpartition->currentText(), ui->repairmountpoint->currentText()))
        {
            ...
            {
                if(! ui->repairmountpoint->currentText().isEmpty()) ui->repairmountpoint->setCurrentText(NULL);
            }
            else
        ...
        if(ui->repairmount->text().isEmpty())
        {
            ui->repairmount->setEnabled(true);
            sb::delay(500);
            ui->repairmount->setIcon(QIcon(NULL));
...
start:;
    ...
    {
        if(isfile("/usr/share/initramfs-tools/scripts/live-bottom/30accessibility"))
        {
    ...
    if(xmntry)
        if(! sb::remove("/usr/share/initramfs-tools/scripts/init-bottom/sbnoxconf")) goto error;
    ...
    if(sb::getarch() == "amd64")
    {
        if(sb::exec("tar -xzf /usr/share/systemback/efi.tar.gz -C " % sb::sdir[2] % "/.sblivesystemcreate") > 0) goto error;
    ...
    if(isdir(sb::sdir[2] % "/.sblivesystemcreate/userdata"))
        if(! sb::remove(sb::sdir[2] % "/.sblivesystemcreate/userdata")) goto error;
...
start:;
    ...
    if(sb::exist(sb::sdir[2] % "/.sblivesystemconvert"))
        if(! sb::remove(sb::sdir[2] % "/.sblivesystemconvert")) goto error;

    if(! QDir().mkdir(sb::sdir[2] % "/.sblivesystemconvert")) goto error;

    if(sb::exec("tar -xf " % sb::sdir[2] % '/' % sb::left(ui->livelist->currentItem()->text(), sb::instr(ui->livelist->currentItem()->text(), " ") - 1) % ".sblive -C " % sb::sdir[2] % "/.sblivesystemconvert --no-same-permissions --no-same-owner") > 0)
    {
        dialog = 47;
        goto error;
    }
